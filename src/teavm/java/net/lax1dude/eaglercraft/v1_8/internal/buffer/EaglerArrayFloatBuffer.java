...
public class EaglerArrayFloatBuffer implements FloatBuffer {
    ...
    public FloatBuffer put(FloatBuffer src) {
        if (src instanceof EaglerArrayFloatBuffer) {
            ...
            Uint8Array.create(dataView.getBuffer()).set(Uint8Array.create(c.dataView.getBuffer(), o + (c.position << SHIFT), (c.limit - c.position) << SHIFT), dataView.getByteOffset() + (position << SHIFT));
            position += l;
            c.position += l;
        } else {
            int l = src.remaining();

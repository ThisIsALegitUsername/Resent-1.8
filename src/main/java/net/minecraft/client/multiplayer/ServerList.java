...
public class ServerList {
    ...
    public void loadServerList() {
        try {
            ...
            if (localStorage != null) {
                NBTTagCompound nbttagcompound = CompressedStreamTools.readCompressed(new EaglerInputStream(localStorage));
    ...
    public static void func_147414_b(ServerData parServerData) {
        ...
        for (int i = 0; i < serverlist.countServers(); ++i) {
            ...
            if (serverdata.serverName.equals(parServerData.serverName) && serverdata.serverIP.equals(parServerData.serverIP)) {
        ...
        while (itr.hasNext()) {
            ...
            if (dat.pingSentTime <= 0l) {
                ...
                if (RateLimitTracker.isLockedOut(dat.serverIP)) {
                    logger.error("Server {} locked this client out on a previous connection, will not attempt to reconnect", dat.serverIP);
                ...
                if (!dat.currentQuery.isOpen() && dat.pingSentTime > 0l && (System.currentTimeMillis() - dat.pingSentTime) > 2000l && !dat.hasPing) {
                    if (RateLimitTracker.isProbablyLockedOut(dat.serverIP)) {
                        logger.error("Server {} ratelimited this client out on a previous connection, assuming lockout", dat.serverIP);
                        dat.serverMOTD = EnumChatFormatting.RED + "Too Many Requests!\nTry again later";
                    }
                    dat.pingToServer = -1l;
                    dat.hasPing = true;

...
public class ItemRenderer {
    ...
    private void func_178098_a(float clientPlayer, AbstractClientPlayer parAbstractClientPlayer) {
        ...
        float f = (float) this.itemToRender.getMaxItemUseDuration() - ((float) parAbstractClientPlayer.getItemInUseCount() - clientPlayer + 1.0F);
        float f1 = f / 20.0F;
    ...
    private void renderFireInFirstPerson(float parFloat1) {
        ...
        for (int i = 0; i < 2; ++i) {
            GlStateManager.pushMatrix();
            EaglerTextureAtlasSprite textureatlassprite = this.mc.getTextureMapBlocks().getAtlasSprite("minecraft:blocks/fire_layer_1");
            this.mc.getTextureManager().bindTexture(TextureMap.locationBlocksTexture);

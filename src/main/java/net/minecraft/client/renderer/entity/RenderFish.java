...
public class RenderFish extends Render<EntityFishHook> {
    ...
    public void doRender(EntityFishHook entityfishhook, double d0, double d1, double d2, float f, float f1) {
        ...
        if (entityfishhook.angler != null) {
            ...
            double d4 = entityfishhook.angler.prevPosY + (entityfishhook.angler.posY - entityfishhook.angler.prevPosY) * (double) f1 + vec3.yCoord;
            ...
            if (this.renderManager.options != null && this.renderManager.options.thirdPersonView > 0 || entityfishhook.angler != Minecraft.getMinecraft().thePlayer) {
                ...
                double d11 = 0.35D;
                double d13 = 0.8D;
                d3 = entityfishhook.angler.prevPosX + (entityfishhook.angler.posX - entityfishhook.angler.prevPosX) * (double) f1 - d9 * 0.35D - d7 * 0.8D;
                d4 = entityfishhook.angler.prevPosY + d6 + (entityfishhook.angler.posY - entityfishhook.angler.prevPosY) * (double) f1 - 0.45D;
                d5 = entityfishhook.angler.prevPosZ + (entityfishhook.angler.posZ - entityfishhook.angler.prevPosZ) * (double) f1 - d7 * 0.35D + d9 * 0.8D;
            ...
            for (int i = 0; i <= 16; ++i) {
                ...
                worldrenderer.pos(d0 + d12 * (double) f12, d1 + d14 * (double) (f12 * f12 + f12) * 0.5D + 0.25D, d2 + d15 * (double) f12).color(0, 0, 0, 255).endVertex();
            }

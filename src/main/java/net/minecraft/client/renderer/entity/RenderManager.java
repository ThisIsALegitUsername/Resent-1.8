...
public class RenderManager {
    ...
    public boolean renderEntityStatic(Entity entity, float partialTicks, boolean parFlag) {
        ...
        return this.doRenderEntity(entity, d0 - this.renderPosX, d1 - this.renderPosY, d2 - this.renderPosZ, f, partialTicks, parFlag);
    }
            ...
            if (render != null && this.renderEngine != null) {
                ...
                if (W.hitboxes().isEnabled() && W.hitboxes().old.getValue() && !entity.isInvisible()) {
                    ...
                    RenderGlobal.func_181563_a(
                        new AxisAlignedBB(
                            entity.boundingBox.minX - 0.05 - entity.posX + (entity.posX - renderPosX),
                            entity.boundingBox.minY - entity.posY + (entity.posY - renderPosY),
                            entity.boundingBox.minZ - entity.posZ + (entity.posZ - renderPosZ),
                            entity.boundingBox.maxX - entity.posX + (entity.posX - renderPosX),
                            entity.boundingBox.maxY - entity.posY + (entity.posY - renderPosY),
                            entity.boundingBox.maxZ - entity.posZ + (entity.posZ - renderPosZ)
                        ),
                        RenderUtils.getColorWithoutRGB(Hitboxes.color).getRed(),
                        RenderUtils.getColorWithoutRGB(Hitboxes.color).getGreen(),
                        RenderUtils.getColorWithoutRGB(Hitboxes.color).getBlue(),
                        255
                    );
     ...
     * pressed
     */
    private void renderDebugBoundingBox(Entity entityIn, double parDouble1, double parDouble2, double parDouble3, float parFloat1, float parFloat2) {
        GlStateManager.depthMask(false);
        GlStateManager.disableTexture2D();
        GlStateManager.disableLighting();
        GlStateManager.disableCull();
        ...
        worldrenderer.pos(parDouble1, parDouble2 + (double) entityIn.getEyeHeight(), parDouble3).color(0, 0, 255, 255).endVertex();
        worldrenderer.pos(parDouble1 + vec3.xCoord * 2.0D, parDouble2 + (double) entityIn.getEyeHeight() + vec3.yCoord * 2.0D, parDouble3 + vec3.zCoord * 2.0D).color(0, 0, 255, 255).endVertex();
        tessellator.draw();
        GlStateManager.enableTexture2D();
        GlStateManager.enableLighting();
        GlStateManager.enableCull();

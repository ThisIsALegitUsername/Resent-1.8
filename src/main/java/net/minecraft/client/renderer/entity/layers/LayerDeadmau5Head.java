...
public class LayerDeadmau5Head implements LayerRenderer<AbstractClientPlayer> {
    ...
    public void doRenderLayer(AbstractClientPlayer abstractclientplayer, float var2, float var3, float f, float var5, float var6, float var7, float var8) {
        if (abstractclientplayer.getName().equals("deadmau5") && abstractclientplayer.hasSkin() && !abstractclientplayer.isInvisible() && this.playerRenderer.getMainModel() instanceof ModelPlayer) {
            this.playerRenderer.bindTexture(abstractclientplayer.getLocationSkin());

            for (int i = 0; i < 2; ++i) {
                float f1 = abstractclientplayer.prevRotationYaw + (abstractclientplayer.rotationYaw - abstractclientplayer.prevRotationYaw) * f - (abstractclientplayer.prevRenderYawOffset + (abstractclientplayer.renderYawOffset - abstractclientplayer.prevRenderYawOffset) * f);

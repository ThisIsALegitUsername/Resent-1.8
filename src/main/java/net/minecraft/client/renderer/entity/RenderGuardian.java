...
public class RenderGuardian extends RenderLiving<EntityGuardian> {
        ...
        if (super.shouldRender(entityguardian, icamera, d0, d1, d2)) {
            ...
            if (entityguardian.hasTargetedEntity()) {
                ...
                if (entitylivingbase != null) {
                    ...
                    if (icamera.isBoundingBoxInFrustum(AxisAlignedBB.fromBounds(vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord, vec3.yCoord, vec3.zCoord))) {
                        return true;
                    }
                }
            }
    ...
    private Vec3 func_177110_a(EntityLivingBase entityLivingBaseIn, double parDouble1, float parFloat1) {
        double d0 = entityLivingBaseIn.lastTickPosX + (entityLivingBaseIn.posX - entityLivingBaseIn.lastTickPosX) * (double) parFloat1;
        double d1 = parDouble1 + entityLivingBaseIn.lastTickPosY + (entityLivingBaseIn.posY - entityLivingBaseIn.lastTickPosY) * (double) parFloat1;
        double d2 = entityLivingBaseIn.lastTickPosZ + (entityLivingBaseIn.posZ - entityLivingBaseIn.lastTickPosZ) * (double) parFloat1;

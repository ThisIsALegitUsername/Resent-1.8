...
public abstract class Render<T extends Entity> {
    ...
    protected void renderOffsetLivingLabel(T entityIn, double x, double y, double z, String str, float parFloat1, double parDouble4) {
    ...
    private void func_180549_a(Block blockIn, double pos, double parDouble2, double parDouble3, BlockPos parBlockPos, float parFloat1, float parFloat2, double parDouble4, double parDouble5, double parDouble6) {
        if (blockIn.isFullCube()) {
            Tessellator tessellator = Tessellator.getInstance();
            WorldRenderer worldrenderer = tessellator.getWorldRenderer();
            double d0 = ((double) parFloat1 - (parDouble2 - ((double) parBlockPos.getY() + parDouble5)) / 2.0D) * 0.5D * (double) this.getWorldFromRenderManager().getLightBrightness(parBlockPos);
            if (d0 >= 0.0D) {
                if (d0 > 1.0D) {
                    d0 = 1.0D;
                }

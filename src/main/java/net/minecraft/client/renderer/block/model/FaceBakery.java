...
public class FaceBakery {
    ...
    public BakedQuad makeBakedQuad(Vector3f posFrom, Vector3f posTo, BlockPartFace face, EaglerTextureAtlasSprite sprite, EnumFacing facing, ModelRotation modelRotationIn, BlockPartRotation partRotation, boolean uvLocked, boolean shade) {
    ...
    private int[] makeQuadVertexData(BlockPartFace partFace, EaglerTextureAtlasSprite sprite, EnumFacing facing, float[] modelRotationIn, ModelRotation partRotation, BlockPartRotation uvLocked, boolean shade, boolean parFlag2) {
        int[] aint = new int[28];

        for (int i = 0; i < 4; ++i) {
            this.fillVertexData(aint, i, facing, partFace, modelRotationIn, sprite, partRotation, uvLocked, shade, parFlag2);
        }
    ...
    private void fillVertexData(int[] faceData, int vertexIndex, EnumFacing facing, BlockPartFace partFace, float[] sprite, EaglerTextureAtlasSprite modelRotationIn, ModelRotation partRotation, BlockPartRotation uvLocked, boolean shade, boolean parFlag2) {
        ...
        Vector3f vector3f = new Vector3f(sprite[enumfacedirection$vertexinformation.field_179184_a], sprite[enumfacedirection$vertexinformation.field_179182_b], sprite[enumfacedirection$vertexinformation.field_179183_c]);
    ...
    private void storeVertexData(int[] faceData, int storeIndex, int vertexIndex, Vector3f position, int shadeColor, EaglerTextureAtlasSprite sprite, BlockFaceUV faceUV) {
        int i = storeIndex * 7;
    ...
    private void func_178407_a(Vector3f partRotation, BlockPartRotation parBlockPartRotation) {
        if (parBlockPartRotation != null) {
            ...
            switch (parBlockPartRotation.axis) {
                case X:
                    Matrix4f.rotate(parBlockPartRotation.angle * 0.017453292F, new Vector3f(1.0F, 0.0F, 0.0F), matrix4f, matrix4f);
                    vector3f.set(0.0F, 1.0F, 1.0F);
                    break;
                case Y:
                    Matrix4f.rotate(parBlockPartRotation.angle * 0.017453292F, new Vector3f(0.0F, 1.0F, 0.0F), matrix4f, matrix4f);
                    vector3f.set(1.0F, 0.0F, 1.0F);
                    break;
                case Z:
                    Matrix4f.rotate(parBlockPartRotation.angle * 0.017453292F, new Vector3f(0.0F, 0.0F, 1.0F), matrix4f, matrix4f);
    ...
    public void func_178409_a(int[] facing, EnumFacing parEnumFacing, BlockFaceUV parBlockFaceUV, EaglerTextureAtlasSprite parTextureAtlasSprite) {
        for (int i = 0; i < 4; ++i) {
    ...
    private void func_178401_a(int facing, int[] parArrayOfInt, EnumFacing parEnumFacing, BlockFaceUV parBlockFaceUV, EaglerTextureAtlasSprite parTextureAtlasSprite) {
        int i = 7 * facing;

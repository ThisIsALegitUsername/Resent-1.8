...
public class ItemModelGenerator {
    ...
    public ModelBlock makeItemModel(TextureMap textureMapIn, ModelBlock blockModel) {
        ...
        for (int i = 0; i < LAYERS.size(); ++i) {
            ...
            hashmap.put(s, s1);
            EaglerTextureAtlasSprite textureatlassprite = textureMapIn.getAtlasSprite((new ResourceLocation(s1)).toString());
    ...
    private List<BlockPart> func_178394_a(int parInt1, String parString1, EaglerTextureAtlasSprite parTextureAtlasSprite) {
        HashMap hashmap = Maps.newHashMap();
        hashmap.put(EnumFacing.SOUTH, new BlockPartFace((EnumFacing) null, parInt1, parString1, new BlockFaceUV(new float[] { 0.0F, 0.0F, 16.0F, 16.0F }, 0)));
        hashmap.put(EnumFacing.NORTH, new BlockPartFace((EnumFacing) null, parInt1, parString1, new BlockFaceUV(new float[] { 16.0F, 0.0F, 0.0F, 16.0F }, 0)));
        ArrayList arraylist = Lists.newArrayList();
    ...
    private List<BlockPart> func_178397_a(EaglerTextureAtlasSprite parTextureAtlasSprite, String parString1, int parInt1) {
        ...
        for (ItemModelGenerator.Span itemmodelgenerator$span : this.func_178393_a(parTextureAtlasSprite)) {
            ...
            f9 = f9 * f11;
            HashMap hashmap = Maps.newHashMap();
            hashmap.put(itemmodelgenerator$spanfacing.getFacing(), new BlockPartFace((EnumFacing) null, parInt1, parString1, new BlockFaceUV(new float[] { f6, f8, f7, f9 }, 0)));
            switch (itemmodelgenerator$spanfacing) {
                case UP:
    ...
    private void func_178396_a(ItemModelGenerator.SpanFacing parSpanFacing, List<ItemModelGenerator.Span> parList, int[] parArrayOfInt, int parInt1, int parInt2, int parInt3, int parInt4, boolean parFlag) {
        boolean flag = this.func_178391_a(parArrayOfInt, parInt1 + parSpanFacing.func_178372_b(), parInt2 + parSpanFacing.func_178371_c(), parInt3, parInt4) && parFlag;
        if (flag) {
            this.func_178395_a(parList, parSpanFacing, parInt1, parInt2);
        }
    }

    private void func_178395_a(List<ItemModelGenerator.Span> parList, ItemModelGenerator.SpanFacing parSpanFacing, int parInt1, int parInt2) {
    ...
    private boolean func_178391_a(int[] parArrayOfInt, int parInt1, int parInt2, int parInt3, int parInt4) {
        return parInt1 >= 0 && parInt2 >= 0 && parInt1 < parInt3 && parInt2 < parInt4 ? (parArrayOfInt[parInt2 * parInt3 + parInt1] >> 24 & 255) == 0 : true;
    }

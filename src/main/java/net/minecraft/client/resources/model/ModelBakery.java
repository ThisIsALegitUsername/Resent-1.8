...
public class ModelBakery {
    ...
    public ModelBakery(IResourceManager parIResourceManager, TextureMap parTextureMap, BlockModelShapes parBlockModelShapes) {
    ...
    private void registerVariant(ModelBlockDefinition parModelBlockDefinition, ModelResourceLocation parModelResourceLocation) {
        this.variants.put(parModelResourceLocation, parModelBlockDefinition.getVariants(parModelResourceLocation.getVariant()));
    }
            ...
            try {
                for (IResource iresource : this.resourceManager.getAllResources(resourcelocation)) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("Encountered an exception when loading model definition of \'" + parResourceLocation + "\' from: \'" + iresource.getResourceLocation() + "\' in resourcepack: \'" + iresource.getResourcePackName() + "\'", exception);
                    } finally {
    ...
    private ResourceLocation getBlockStateLocation(ResourceLocation parResourceLocation) {
        return new ResourceLocation(parResourceLocation.getResourceDomain(), "blockstates/" + parResourceLocation.getResourcePath() + ".json");
    }
    ...
    private ResourceLocation getModelLocation(ResourceLocation parResourceLocation) {
        return new ResourceLocation(parResourceLocation.getResourceDomain(), "models/" + parResourceLocation.getResourcePath() + ".json");
    }
            ...
            for (String s : this.getVariantNames(item)) {
                ...
                if (this.models.get(resourcelocation) == null) {
                    try {
                        ...
                        LOGGER.warn("Unable to load item model: \'" + resourcelocation + "\' for item: \'" + Item.itemRegistry.getNameForObject(item) + "\'");
                        LOGGER.warn(exception);
                    }
                }
            }
    ...
    private List<String> getVariantNames(Item parItem) {
        ...
        if (list == null) {
            list = Collections.singletonList(((ResourceLocation) Item.itemRegistry.getNameForObject(parItem)).toString());
        }
    ...
    private Set<ResourceLocation> getVariantsTextureLocations() {
        ...
        Collections.sort(
            ...
            new Comparator<ModelResourceLocation>() {
                public int compare(ModelResourceLocation modelresourcelocation1, ModelResourceLocation modelresourcelocation2) {
    ...
    private IBakedModel bakeModel(ModelBlock modelBlockIn, ModelRotation modelRotationIn, boolean uvLocked) {
        EaglerTextureAtlasSprite textureatlassprite = (EaglerTextureAtlasSprite) this.sprites.get(new ResourceLocation(modelBlockIn.resolveTextureName("particle")));
        ...
        for (BlockPart blockpart : modelBlockIn.getElements()) {
            for (EnumFacing enumfacing : blockpart.mapFaces.keySet()) {
                ...
                EaglerTextureAtlasSprite textureatlassprite1 = (EaglerTextureAtlasSprite) this.sprites.get(new ResourceLocation(modelBlockIn.resolveTextureName(blockpartface.texture)));
    ...
    private BakedQuad makeBakedQuad(BlockPart parBlockPart, BlockPartFace parBlockPartFace, EaglerTextureAtlasSprite parTextureAtlasSprite, EnumFacing parEnumFacing, ModelRotation parModelRotation, boolean parFlag) {
        return this.faceBakery.makeBakedQuad(parBlockPart.positionFrom, parBlockPart.positionTo, parBlockPartFace, parTextureAtlasSprite, parEnumFacing, parModelRotation, parBlockPart.partRotation, parFlag, parBlockPart.shade);
    }
    ...
    private Set<ResourceLocation> getTextureLocations(ModelBlock parModelBlock) {
        ...
        for (BlockPart blockpart : parModelBlock.getElements()) {
            for (BlockPartFace blockpartface : blockpart.mapFaces.values()) {
                ResourceLocation resourcelocation = new ResourceLocation(parModelBlock.resolveTextureName(blockpartface.texture));
                hashset.add(resourcelocation);
            }
        }

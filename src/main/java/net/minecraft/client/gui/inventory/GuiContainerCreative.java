...
public class GuiContainerCreative extends InventoryEffectRenderer {
    ...
    protected void handleMouseClick(Slot slot, int i, int j, int k) {
        ...
        if (slot == null && selectedTabIndex != CreativeTabs.tabInventory.getTabIndex() && k != 5) {
            ...
            if (slot == this.field_147064_C) {
                ...
                this.mc.thePlayer.inventoryContainer.slotClick(slot == null ? i : ((GuiContainerCreative.CreativeSlot) slot).slot.slotNumber, j, k, this.mc.thePlayer);
            ...
            if (Container.getDragEvent(j) == 2) {
                ...
                this.mc.playerController.sendSlotPacket(itemstack4, slot.slotNumber - this.inventorySlots.inventorySlots.size() + 9 + 36);
            }
        }
    }
    ...
    protected int getCloseKey() {
        return selectedTabIndex != CreativeTabs.tabAllSearch.getTabIndex() ? super.getCloseKey() : mc.gameSettings.keyBindClose.getKeyCode();
    }
    ...
    private void setCurrentCreativeTab(CreativeTabs parCreativeTabs) {
        ...
        if (parCreativeTabs == CreativeTabs.tabInventory) {
            ...
            for (int j = 0; j < container.inventorySlots.size(); ++j) {
                GuiContainerCreative.CreativeSlot guicontainercreative$creativeslot = new GuiContainerCreative.CreativeSlot((Slot) container.inventorySlots.get(j), j);
    ...
    public void drawScreen(int i, int j, float f) {
        ...
        if (this.field_147064_C != null && selectedTabIndex == CreativeTabs.tabInventory.getTabIndex() && this.isPointInRegion(this.field_147064_C.xDisplayPosition, this.field_147064_C.yDisplayPosition, 16, 16, i, j)) {
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int i, int j) {
        ...
        if (creativetabs.shouldHidePlayerInventory()) {
            this.drawTexturedModalRect(k, l + (int) ((float) (i1 - l - 17) * this.currentScroll), 232 + (this.needsScrollBars() ? 0 : 12), 0, 12, 15);
        }

        this.func_147051_a(creativetabs);
        if (creativetabs == CreativeTabs.tabInventory) {
            GuiInventory.drawEntityOnScreen(this.guiLeft + 43, this.guiTop + 45, 20, (float) (this.guiLeft + 43 - i), (float) (this.guiTop + 45 - 30 - j), this.mc.thePlayer);
        }
    }
    ...
    protected boolean renderCreativeInventoryHoveringText(CreativeTabs parCreativeTabs, int parInt1, int parInt2) {
        ...
        if (this.isPointInRegion(j + 3, k + 3, 23, 27, parInt1, parInt2)) {
            this.drawCreativeTabHoveringText(I18n.format(parCreativeTabs.getTranslatedTabLabel(), new Object[0]), parInt1, parInt2);
            return true;
        } else {
            return false;
        }
    ...
    static class ContainerCreative extends Container {
        ...
        public boolean canDragIntoSlot(Slot parSlot) {
            return parSlot.inventory instanceof InventoryPlayer || parSlot.yDisplayPosition > 90 && parSlot.xDisplayPosition <= 162;
        }
    }

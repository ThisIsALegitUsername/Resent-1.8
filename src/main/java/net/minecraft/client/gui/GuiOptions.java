...
public class GuiOptions extends GuiScreen implements GuiYesNoCallback {
	...
	public void initGui() {
		...
		for (GameSettings.Options gamesettings$options : field_146440_f) {
			if (gamesettings$options.getEnumFloat()) {
				this.buttonList.add(new GuiOptionSlider(gamesettings$options.returnEnumOrdinal(),
                        width / 2 - 155 + i % 2 * 160, height / 6 - 12 + 24 * (i >> 1),
                        gamesettings$options));
			} else {
				GuiOptionButton guioptionbutton = new GuiOptionButton(gamesettings$options.returnEnumOrdinal(),
                        width / 2 - 155 + i % 2 * 160, height / 6 - 12 + 24 * (i >> 1), gamesettings$options,
		...
		if (this.mc.theWorld != null) {
            ...
            this.field_175357_i = new GuiButton(108, width / 2 - 155 + i % 2 * 160,
                    height / 6 - 12 + 24 * (i >> 1), 150, 20, this.func_175355_a(enumdifficulty));
        ...
        this.buttonList.add(new GuiButton(110, width / 2 - 155, height / 6 + 48 - 6, 150, 20,
                I18n.format("options.skinCustomisation")));
        this.buttonList.add(notSoSuperSecret = new GuiButton(8675309, width / 2 + 5, height / 6 + 48 - 6, 150,
        ...
        this.buttonList.add(new GuiButton(106, width / 2 - 155, height / 6 + 72 - 6, 150, 20,
                I18n.format("options.sounds")));
        this.buttonList.add(broadcastSettings = new GuiButton(107, width / 2 + 5, height / 6 + 72 - 6, 150,
                20, I18n.format(EagRuntime.getRecText())));
        broadcastSettings.enabled = EagRuntime.recSupported();
        this.buttonList.add(new GuiButton(101, width / 2 - 155, height / 6 + 96 - 6, 150, 20,
                I18n.format("options.video")));
        this.buttonList.add(new GuiButton(100, width / 2 + 5, height / 6 + 96 - 6, 150, 20,
                I18n.format("options.controls")));
        this.buttonList.add(new GuiButton(102, width / 2 - 155, height / 6 + 120 - 6, 150, 20,
                I18n.format("options.language")));
        this.buttonList.add(new GuiButton(103, width / 2 + 5, height / 6 + 120 - 6, 150, 20,
                I18n.format("options.chat.title")));
        GuiButton rp;
        this.buttonList.add(rp = new GuiButton(105, width / 2 - 155, height / 6 + 144 - 6, 150, 20,
                I18n.format("options.resourcepack")));
        GuiButton b;
        this.buttonList.add(b = new GuiButton(104, width / 2 + 5, height / 6 + 144 - 6, 150, 20,
	...
	public String func_175355_a(EnumDifficulty parEnumDifficulty) {
        ...
        chatcomponenttext.appendSibling(new ChatComponentTranslation("options.difficulty"));
        chatcomponenttext.appendText(": ");
        chatcomponenttext.appendSibling(
                new ChatComponentTranslation(parEnumDifficulty.getDifficultyResourceKey()));
	...
	protected void actionPerformed(GuiButton parGuiButton) {
		if (parGuiButton.enabled) {
			...
			if (parGuiButton.id == 107) {
                ...
                broadcastSettings.displayString = I18n.format(EagRuntime.getRecText());
			}
		}
	}

...
public class GuiAchievements extends GuiScreen implements IProgressMeter {
    ...
    public void drawScreen(int i, int j, float f) {
        if (this.loadingAchievements) {
            ...
            if (Mouse.isButtonDown(0)) {
                ...
                int i1 = k + 8;
                int j1 = l + 17;
                if ((this.field_146554_D == 0 || this.field_146554_D == 1) && i >= i1 && i < i1 + 224 && j >= j1 && j < j1 + 155) {
    ...
    protected void drawAchievementScreen(int parInt1, int parInt2, float parFloat1) {
        int i = MathHelper.floor_double(this.field_146569_s + (this.field_146567_u - this.field_146569_s) * (double) parFloat1);
        int j = MathHelper.floor_double(this.field_146568_t + (this.field_146566_v - this.field_146568_t) * (double) parFloat1);
        if (i < field_146572_y) {
            i = field_146572_y;
        }
        ...
        for (int k2 = 0; (float) k2 * f - (float) j2 < 155.0F; ++k2) {
            ...
            for (int l2 = 0; (float) l2 * f1 - (float) i2 < 224.0F; ++l2) {
                random.setSeed((long) (this.mc.getSession().getProfile().getId().hashCode() + k1 + l2 + (l1 + k2) * 16));
        ...
        for (int l4 = 0; l4 < AchievementList.achievementList.size(); ++l4) {
            ...
            if (k5 >= -24 && i6 >= -24 && (float) k5 <= 224.0F * this.field_146570_r && (float) i6 <= 155.0F * this.field_146570_r) {
        ...
        if (achievement != null) {
            ...
            if (this.statFileWriter.canUnlockAchievement(achievement)) {
                ...
                if (this.statFileWriter.hasAchievementUnlocked(achievement)) {
                    this.fontRendererObj.drawStringWithShadow(I18n.format("achievement.taken", new Object[0]), (float) l5, (float) (j6 + l7 + 4), -7302913);
                }
            } else if (l6 == 3) {
            ...
            if (s != null) {
                this.fontRendererObj.drawStringWithShadow(s, (float) l5, (float) j6, this.statFileWriter.canUnlockAchievement(achievement) ? (achievement.getSpecial() ? -128 : -1) : (achievement.getSpecial() ? -8355776 : -8355712));
            }
        }
    ...
    private EaglerTextureAtlasSprite func_175371_a(Block parBlock) {
        return Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes().getTexture(parBlock.getDefaultState());
    }

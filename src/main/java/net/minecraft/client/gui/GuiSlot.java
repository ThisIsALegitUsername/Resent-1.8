...
public abstract class GuiSlot {
    ...
    public int getSlotIndexFromScreenCoords(int parInt1, int parInt2) {
        ...
        return parInt1 < this.getScrollBarX() && parInt1 >= i && parInt1 <= j && l >= 0 && k >= 0 && l < this.getSize() ? l : -1;
    }
    ...
    protected void drawSelectionBox(int mouseXIn, int mouseYIn, int parInt3, int parInt4) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            try {
                ...
                excLogger.error("Exception caught rendering a slot of a list on the screen! Game will continue running due to the suspicion that this could be an intentional crash attempt, and therefore it would be inconvenient if the user were to be locked out of this gui due to repeatedly triggering a full crash report");
                excLogger.error(t);
            }
        }
    }

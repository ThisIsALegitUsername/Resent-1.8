...
public class EffectRenderer {
    ...
    public EntityFX spawnEffectParticle(int particleId, double parDouble1, double parDouble2, double parDouble3, double parDouble4, double parDouble5, double parDouble6, int... parArrayOfInt) {
        ...
        if (iparticlefactory != null) {
            EntityFX entityfx = iparticlefactory.getEntityFX(particleId, this.worldObj, parDouble1, parDouble2, parDouble3, parDouble4, parDouble5, parDouble6, parArrayOfInt);
            if (entityfx != null) {
            ...
            for (int j = 1; j >= 0; --j) {
                if (!this.fxLayers[i][j].isEmpty()) {
                    ...
                    for (int k = 0; k < this.fxLayers[i][j].size(); ++k) {
                        ...
                        try {
                            if (!entityfx.renderAccelerated(acceleratedParticleRenderer, entityIn, partialTicks, f, f4, f1, f2, f3)) {
                            ...
                            crashreportcategory.addCrashSectionCallable(
                                ...
                                new Callable<String>() {
                                    public String call() throws Exception {
                                        return l == 0 ? "MISC_TEXTURE" : (l == 1 ? "TERRAIN_TEXTURE" : (l == 3 ? "ENTITY_PARTICLE_TEXTURE" : "Unknown - " + l));
                                    }
                                }
                            );
                            throw new ReportedException(crashreport);

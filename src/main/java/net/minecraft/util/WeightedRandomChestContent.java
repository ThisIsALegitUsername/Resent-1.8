...
public class WeightedRandomChestContent extends WeightedRandom.Item {
    ...
    public WeightedRandomChestContent(Item parItem, int parInt1, int minimumChance, int maximumChance, int itemWeightIn) {
        super(itemWeightIn);
    ...
    public static void generateChestContents(EaglercraftRandom random, List<WeightedRandomChestContent> listIn, IInventory inv, int max) {
        for (int i = 0; i < max; ++i) {
    ...
    public static void generateDispenserContents(EaglercraftRandom random, List<WeightedRandomChestContent> listIn, TileEntityDispenser dispenser, int max) {
        for (int i = 0; i < max; ++i) {
            WeightedRandomChestContent weightedrandomchestcontent = (WeightedRandomChestContent) WeightedRandom.getRandomItem(random, listIn);
            int j = weightedrandomchestcontent.minStackSize + random.nextInt(weightedrandomchestcontent.maxStackSize - weightedrandomchestcontent.minStackSize + 1);
            if (weightedrandomchestcontent.theItemId.getMaxStackSize() >= j) {
    ...
    public static List<WeightedRandomChestContent> func_177629_a(List<WeightedRandomChestContent> parList, WeightedRandomChestContent... parArrayOfWeightedRandomChestContent) {

...
public class ItemArmor extends Item {
    ...
    private static final IBehaviorDispenseItem dispenserBehavior = new BehaviorDefaultDispenseItem() {
        protected ItemStack dispenseStack(IBlockSource iblocksource, ItemStack itemstack) {
            ...
            List list = iblocksource.getWorld().getEntitiesWithinAABB(EntityLivingBase.class, axisalignedbb, Predicates.and(EntitySelectors.NOT_SPECTATING, new EntitySelectors.ArmoredMob(itemstack)));
            if (list.size() > 0) {
    ...
    public boolean hasColor(ItemStack parItemStack) {
        return this.material != ItemArmor.ArmorMaterial.LEATHER ? false : (!parItemStack.hasTagCompound() ? false : (!parItemStack.getTagCompound().hasKey("display", 10) ? false : parItemStack.getTagCompound().getCompoundTag("display").hasKey("color", 3)));
    }

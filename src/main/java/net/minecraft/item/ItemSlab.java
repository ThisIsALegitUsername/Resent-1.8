    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, BlockPos blockpos, EnumFacing enumfacing, float f, float f1, float f2) {
        if (itemstack.stackSize == 0) {
            ...
            if (iblockstate.getBlock() == this.singleSlab) {
                ...
                if ((enumfacing == EnumFacing.UP && blockslab$enumblockhalf == BlockSlab.EnumBlockHalf.BOTTOM || enumfacing == EnumFacing.DOWN && blockslab$enumblockhalf == BlockSlab.EnumBlockHalf.TOP) && comparable == object) {
                    IBlockState iblockstate1 = this.doubleSlab.getDefaultState().withProperty(iproperty, comparable);
                    if (world.checkNoEntityCollision(this.doubleSlab.getCollisionBoundingBox(world, blockpos, iblockstate1)) && world.setBlockState(blockpos, iblockstate1, 3)) {
                        world.playSoundEffect((double) ((float) blockpos.getX() + 0.5F), (double) ((float) blockpos.getY() + 0.5F), (double) ((float) blockpos.getZ() + 0.5F), this.doubleSlab.stepSound.getPlaceSound(), (this.doubleSlab.stepSound.getVolume() + 1.0F) / 2.0F, this.doubleSlab.stepSound.getFrequency() * 0.8F);
                        --itemstack.stackSize;
                    }
            ...
            return this.tryPlace(itemstack, world, blockpos.offset(enumfacing), object) ? true : super.onItemUse(itemstack, entityplayer, world, blockpos, enumfacing, f, f1, f2);
        }
    }

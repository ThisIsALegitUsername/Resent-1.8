...
public class BlockRedstoneWire extends Block {
    ...
    public static void bootstrapStates() {
        NORTH = PropertyEnum.<BlockRedstoneWire.EnumAttachPosition>create("north", BlockRedstoneWire.EnumAttachPosition.class);
        EAST = PropertyEnum.<BlockRedstoneWire.EnumAttachPosition>create("east", BlockRedstoneWire.EnumAttachPosition.class);
        SOUTH = PropertyEnum.<BlockRedstoneWire.EnumAttachPosition>create("south", BlockRedstoneWire.EnumAttachPosition.class);
        WEST = PropertyEnum.<BlockRedstoneWire.EnumAttachPosition>create("west", BlockRedstoneWire.EnumAttachPosition.class);
    }
    ...
    public int colorMultiplier(IBlockAccess iblockaccess, BlockPos blockpos, int i) {
        ...
        return iblockstate.getBlock() != this ? super.colorMultiplier(iblockaccess, blockpos, i) : this.colorMultiplier(((Integer) iblockstate.getValue(POWER)).intValue());
    }
        ...
        if (!this.canProvidePower) {
            ...
            if (i == 0) {
                ...
                if (enumfacing.getAxis().isHorizontal() && enumset.isEmpty()) {
                    return i;
                } else if (enumset.contains(enumfacing) && !enumset.contains(enumfacing.rotateYCCW()) && !enumset.contains(enumfacing.rotateY())) {
                    return i;
                } else {
                    return 0;
                }

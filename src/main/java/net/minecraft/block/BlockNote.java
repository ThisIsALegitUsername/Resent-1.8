...
public class BlockNote extends BlockContainer {
    ...
    public boolean onBlockEventReceived(World world, BlockPos blockpos, IBlockState var3, int i, int j) {
        ...
        world.playSoundEffect((double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.5D, (double) blockpos.getZ() + 0.5D, "note." + this.getInstrument(i), 3.0F, f);
        world.spawnParticle(EnumParticleTypes.NOTE, (double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 1.2D, (double) blockpos.getZ() + 0.5D, (double) j / 24.0D, 0.0D, 0.0D, new int[0]);
        return true;
    }

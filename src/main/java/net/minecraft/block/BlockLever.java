...
public class BlockLever extends Block {
     ...
     */
    public IBlockState onBlockPlaced(World world, BlockPos blockpos, EnumFacing enumfacing, float var4, float var5, float var6, int var7, EntityLivingBase entitylivingbase) {
        ...
        if (func_181090_a(world, blockpos, enumfacing.getOpposite())) {
            return iblockstate.withProperty(FACING, BlockLever.EnumOrientation.forFacings(enumfacing, entitylivingbase.getHorizontalFacing()));
        } else {
            for (EnumFacing enumfacing1 : EnumFacing.Plane.HORIZONTAL) {
                if (enumfacing1 != enumfacing && func_181090_a(world, blockpos, enumfacing1.getOpposite())) {
                    return iblockstate.withProperty(FACING, BlockLever.EnumOrientation.forFacings(enumfacing1, entitylivingbase.getHorizontalFacing()));
                }
            }

            if (World.doesBlockHaveSolidTopSurface(world, blockpos.down())) {
                return iblockstate.withProperty(FACING, BlockLever.EnumOrientation.forFacings(EnumFacing.UP, entitylivingbase.getHorizontalFacing()));
            } else {
                return iblockstate;
            }
        }
    ...
    public void onNeighborBlockChange(World world, BlockPos blockpos, IBlockState iblockstate, Block var4) {
        if (this.func_181091_e(world, blockpos, iblockstate) && !func_181090_a(world, blockpos, ((BlockLever.EnumOrientation) iblockstate.getValue(FACING)).getFacing().getOpposite())) {
    ...
    public int getStrongPower(IBlockAccess var1, BlockPos var2, IBlockState iblockstate, EnumFacing enumfacing) {
        return !((Boolean) iblockstate.getValue(POWERED)).booleanValue() ? 0 : (((BlockLever.EnumOrientation) iblockstate.getValue(FACING)).getFacing() == enumfacing ? 15 : 0);
    }

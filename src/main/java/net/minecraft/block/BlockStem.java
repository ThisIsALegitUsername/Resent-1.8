...
public class BlockStem extends BlockBush implements IGrowable {
        ...
        if (world.getLightFromNeighbors(blockpos.up()) >= 9) {
            ...
            if (random.nextInt((int) (25.0F / f) + 1) == 0) {
                ...
                if (i < 7) {
                    ...
                    if (world.getBlockState(blockpos).getBlock().blockMaterial == Material.air && (block == Blocks.farmland || block == Blocks.dirt || block == Blocks.grass)) {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess iblockaccess, BlockPos blockpos) {
        this.maxY = (double) ((float) (((Integer) iblockaccess.getBlockState(blockpos).getValue(AGE)).intValue() * 2 + 2) / 16.0F);
        float f = 0.125F;

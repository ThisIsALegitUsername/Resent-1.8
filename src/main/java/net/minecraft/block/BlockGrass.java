    ...
    public void grow(World world, EaglercraftRandom random, BlockPos blockpos, IBlockState var4) {
        ...
        for (int i = 0; i < 128; ++i) {
            ...
            while (true) {
                if (j >= i / 16) {
                    if (world.getBlockState(blockpos2).getBlock().blockMaterial == Material.air) {
                        if (random.nextInt(8) == 0) {
                            BlockFlower.EnumFlowerType blockflower$enumflowertype = world.getBiomeGenForCoords(blockpos2).pickRandomFlower(random, blockpos2);
                ...
                if (world.getBlockState(blockpos2.down()).getBlock() != Blocks.grass || world.getBlockState(blockpos2).getBlock().isNormalCube()) {
                    break;
                }

...
public class BlockPortal extends BlockBreakable {
    ...
    public void updateTick(World world, BlockPos blockpos, IBlockState iblockstate, EaglercraftRandom random) {
        ...
        if (world.provider.isSurfaceWorld() && world.getGameRules().getBoolean("doMobSpawning") && random.nextInt(2000) < world.getDifficulty().getDifficultyId()) {
            ...
            BlockPos blockpos1;
            for (blockpos1 = blockpos; !World.doesBlockHaveSolidTopSurface(world, blockpos1) && blockpos1.getY() > 0; blockpos1 = blockpos1.down()) {}

            if (i > 0 && !world.getBlockState(blockpos1.up()).getBlock().isNormalCube()) {
                Entity entity = ItemMonsterPlacer.spawnCreature(world, 57, (double) blockpos1.getX() + 0.5D, (double) blockpos1.getY() + 1.1D, (double) blockpos1.getZ() + 0.5D);
                if (entity != null) {
    ...
    public boolean shouldSideBeRendered(IBlockAccess iblockaccess, BlockPos blockpos, EnumFacing enumfacing) {
        ...
        if (iblockaccess.getBlockState(blockpos).getBlock() == this) {
            ...
            if (enumfacing$axis == EnumFacing.Axis.Z && enumfacing != EnumFacing.EAST && enumfacing != EnumFacing.WEST) {
                return false;
            }

            if (enumfacing$axis == EnumFacing.Axis.X && enumfacing != EnumFacing.SOUTH && enumfacing != EnumFacing.NORTH) {
                return false;
            }
        }

        boolean flag = iblockaccess.getBlockState(blockpos.west()).getBlock() == this && iblockaccess.getBlockState(blockpos.west(2)).getBlock() != this;
        boolean flag1 = iblockaccess.getBlockState(blockpos.east()).getBlock() == this && iblockaccess.getBlockState(blockpos.east(2)).getBlock() != this;
        boolean flag2 = iblockaccess.getBlockState(blockpos.north()).getBlock() == this && iblockaccess.getBlockState(blockpos.north(2)).getBlock() != this;
        boolean flag3 = iblockaccess.getBlockState(blockpos.south()).getBlock() == this && iblockaccess.getBlockState(blockpos.south(2)).getBlock() != this;
        boolean flag4 = flag || flag1 || enumfacing$axis == EnumFacing.Axis.X;
        boolean flag5 = flag2 || flag3 || enumfacing$axis == EnumFacing.Axis.Z;
        return flag4 && enumfacing == EnumFacing.WEST ? true : (flag4 && enumfacing == EnumFacing.EAST ? true : (flag5 && enumfacing == EnumFacing.NORTH ? true : flag5 && enumfacing == EnumFacing.SOUTH));
    }
    ...
    public void randomDisplayTick(World world, BlockPos blockpos, IBlockState var3, EaglercraftRandom random) {
        if (random.nextInt(100) == 0) {
            world.playSound((double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.5D, (double) blockpos.getZ() + 0.5D, "portal.portal", 0.5F, random.nextFloat() * 0.4F + 0.8F, false);
        }
    ...
    public static class Size {
        ...
        public Size(World worldIn, BlockPos parBlockPos, EnumFacing.Axis parAxis) {
            ...
            for (BlockPos blockpos = parBlockPos; parBlockPos.getY() > blockpos.getY() - 21 && parBlockPos.getY() > 0 && this.func_150857_a(worldIn.getBlockState(parBlockPos.down()).getBlock()); parBlockPos = parBlockPos.down()) {}

...
public class BlockFire extends Block {
    ...
    public IBlockState getActualState(IBlockState iblockstate, IBlockAccess iblockaccess, BlockPos blockpos) {
        ...
        if (!World.doesBlockHaveSolidTopSurface(iblockaccess, blockpos.down()) && !Blocks.fire.canCatchFire(iblockaccess, blockpos.down())) {
            ...
            return iblockstate
                .withProperty(NORTH, Boolean.valueOf(this.canCatchFire(iblockaccess, blockpos.north())))
                        ...
                        for (int l = -1; l <= 4; ++l) {
                            if (j != 0 || l != 0 || k != 0) {
                                ...
                                if (j1 > 0) {
                                    ...
                                    if (k1 > 0 && random.nextInt(i1) <= k1 && (!world.isRaining() || !this.canDie(world, blockpos1))) {
                                        int l1 = i + random.nextInt(5) / 4;
                                        if (l1 > 15) {
                                            l1 = 15;
                                        }

                                        world.setBlockState(blockpos1, iblockstate.withProperty(AGE, Integer.valueOf(l1)), 3);
                                    }
                                }
                            }
                        }
    ...
    public void randomDisplayTick(World world, BlockPos blockpos, IBlockState var3, EaglercraftRandom random) {
        if (random.nextInt(24) == 0) {
            world.playSound((double) ((float) blockpos.getX() + 0.5F), (double) ((float) blockpos.getY() + 0.5F), (double) ((float) blockpos.getZ() + 0.5F), "fire.fire", 1.0F + random.nextFloat(), random.nextFloat() * 0.7F + 0.3F, false);
        }

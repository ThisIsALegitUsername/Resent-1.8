...
public abstract class BlockLiquid extends Block {
    ...
    public int colorMultiplier(IBlockAccess iblockaccess, BlockPos blockpos, int var3) {
        return this.blockMaterial == Material.water ? BiomeColorHelper.getWaterColorAtPos(iblockaccess, blockpos) : 16777215;
    }
        ...
        if (this.blockMaterial == Material.water) {
            ...
            if (i > 0 && i < 8) {
                if (random.nextInt(64) == 0) {
                    world.playSound(d0 + 0.5D, d1 + 0.5D, d2 + 0.5D, "liquid.water", random.nextFloat() * 0.25F + 0.75F, random.nextFloat() * 1.0F + 0.5F, false);
                }
            } else if (random.nextInt(10) == 0) {
                world.spawnParticle(EnumParticleTypes.SUSPENDED, d0 + (double) random.nextFloat(), d1 + (double) random.nextFloat(), d2 + (double) random.nextFloat(), 0.0D, 0.0D, 0.0D, new int[0]);
            }
        }

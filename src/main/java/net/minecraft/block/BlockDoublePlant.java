...
public class BlockDoublePlant extends BlockBush implements IGrowable {
    ...
    public boolean isReplaceable(World world, BlockPos blockpos) {
        ...
        if (iblockstate.getBlock() != this) {
            ...
            BlockDoublePlant.EnumPlantType blockdoubleplant$enumplanttype = (BlockDoublePlant.EnumPlantType) this.getActualState(iblockstate, world, blockpos).getValue(VARIANT);
    ...
    public Item getItemDropped(IBlockState iblockstate, EaglercraftRandom random, int var3) {
        if (iblockstate.getValue(HALF) == BlockDoublePlant.EnumBlockHalf.UPPER) {
            ...
            return blockdoubleplant$enumplanttype == BlockDoublePlant.EnumPlantType.FERN ? null : (blockdoubleplant$enumplanttype == BlockDoublePlant.EnumPlantType.GRASS ? (random.nextInt(8) == 0 ? Items.wheat_seeds : null) : Item.getItemFromBlock(this));
        }
    }
    ...
    public int damageDropped(IBlockState iblockstate) {
        return iblockstate.getValue(HALF) != BlockDoublePlant.EnumBlockHalf.UPPER && iblockstate.getValue(VARIANT) != BlockDoublePlant.EnumPlantType.GRASS ? ((BlockDoublePlant.EnumPlantType) iblockstate.getValue(VARIANT)).getMeta() : 0;
    }
    ...
    public int colorMultiplier(IBlockAccess iblockaccess, BlockPos blockpos, int var3) {
        ...
        return blockdoubleplant$enumplanttype != BlockDoublePlant.EnumPlantType.GRASS && blockdoubleplant$enumplanttype != BlockDoublePlant.EnumPlantType.FERN ? 16777215 : BiomeColorHelper.getGrassColorAtPos(iblockaccess, blockpos);
    }
    ...
    public void onBlockPlacedBy(World world, BlockPos blockpos, IBlockState var3, EntityLivingBase var4, ItemStack var5) {
        world.setBlockState(blockpos.up(), this.getDefaultState().withProperty(HALF, BlockDoublePlant.EnumBlockHalf.UPPER), 2);
    }
        ...
        if (iblockstate.getValue(HALF) == BlockDoublePlant.EnumBlockHalf.UPPER) {
            if (world.getBlockState(blockpos.down()).getBlock() == this) {
                if (!entityplayer.capabilities.isCreativeMode) {
                    ...
                    BlockDoublePlant.EnumPlantType blockdoubleplant$enumplanttype = (BlockDoublePlant.EnumPlantType) iblockstate1.getValue(VARIANT);
    ...
    public IBlockState getStateFromMeta(int i) {
        return (i & 8) > 0 ? this.getDefaultState().withProperty(HALF, BlockDoublePlant.EnumBlockHalf.UPPER) : this.getDefaultState().withProperty(HALF, BlockDoublePlant.EnumBlockHalf.LOWER).withProperty(VARIANT, BlockDoublePlant.EnumPlantType.byMetadata(i & 7));
    }
    ...
    public int getMetaFromState(IBlockState iblockstate) {
        return iblockstate.getValue(HALF) == BlockDoublePlant.EnumBlockHalf.UPPER ? 8 | ((EnumFacing) iblockstate.getValue(field_181084_N)).getHorizontalIndex() : ((BlockDoublePlant.EnumPlantType) iblockstate.getValue(VARIANT)).getMeta();
    }

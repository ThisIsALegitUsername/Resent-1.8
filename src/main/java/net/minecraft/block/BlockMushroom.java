...
public class BlockMushroom extends BlockBush implements IGrowable {
    ...
    public boolean canBlockStay(World world, BlockPos blockpos, IBlockState var3) {
        if (blockpos.getY() >= 0 && blockpos.getY() < 256) {
            ...
            return iblockstate.getBlock() == Blocks.mycelium ? true : (iblockstate.getBlock() == Blocks.dirt && iblockstate.getValue(BlockDirt.VARIANT) == BlockDirt.DirtType.PODZOL ? true : world.getLight(blockpos) < 13 && this.canPlaceBlockOn(iblockstate.getBlock()));
        } else {
            return false;
        }
    }

...
public class BlockDoor extends Block {
    ...
    public boolean onBlockActivated(World world, BlockPos blockpos, IBlockState iblockstate, EntityPlayer entityplayer, EnumFacing var5, float var6, float var7, float var8) {
        if (this.blockMaterial == Material.iron) {
            ...
            BlockPos blockpos1 = iblockstate.getValue(HALF) == BlockDoor.EnumDoorHalf.LOWER ? blockpos : blockpos.down();
    ...
    public void onNeighborBlockChange(World world, BlockPos blockpos, IBlockState iblockstate, Block block) {
        if (iblockstate.getValue(HALF) == BlockDoor.EnumDoorHalf.UPPER) {
            ...
            if (!flag1) {
                ...
                if ((flag || block.canProvidePower()) && block != this && flag != ((Boolean) iblockstate2.getValue(POWERED)).booleanValue()) {
    ...
    public void onBlockHarvested(World world, BlockPos blockpos, IBlockState iblockstate, EntityPlayer entityplayer) {
        BlockPos blockpos1 = blockpos.down();
        if (entityplayer.capabilities.isCreativeMode && iblockstate.getValue(HALF) == BlockDoor.EnumDoorHalf.UPPER && world.getBlockState(blockpos1).getBlock() == this) {
    ...
    public IBlockState getActualState(IBlockState iblockstate, IBlockAccess iblockaccess, BlockPos blockpos) {
        if (iblockstate.getValue(HALF) == BlockDoor.EnumDoorHalf.LOWER) {
            ...
            if (iblockstate1.getBlock() == this) {
                iblockstate = iblockstate.withProperty(HINGE, iblockstate1.getValue(HINGE)).withProperty(POWERED, iblockstate1.getValue(POWERED));
            }
        } else {
            IBlockState iblockstate2 = iblockaccess.getBlockState(blockpos.down());
            if (iblockstate2.getBlock() == this) {
                iblockstate = iblockstate.withProperty(FACING, iblockstate2.getValue(FACING)).withProperty(OPEN, iblockstate2.getValue(OPEN));
            }
        }
    ...
    public IBlockState getStateFromMeta(int i) {
        return (i & 8) > 0
            ? this.getDefaultState().withProperty(HALF, BlockDoor.EnumDoorHalf.UPPER).withProperty(HINGE, (i & 1) > 0 ? BlockDoor.EnumHingePosition.RIGHT : BlockDoor.EnumHingePosition.LEFT).withProperty(POWERED, Boolean.valueOf((i & 2) > 0))
            : this.getDefaultState().withProperty(HALF, BlockDoor.EnumDoorHalf.LOWER).withProperty(FACING, EnumFacing.getHorizontal(i & 3).rotateYCCW()).withProperty(OPEN, Boolean.valueOf((i & 4) > 0));
    }

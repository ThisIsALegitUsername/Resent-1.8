...
public class InventoryPlayer implements IInventory {
    ...
    private int getInventorySlotContainItemAndDamage(Item itemIn, int parInt1) {
        for (int i = 0; i < this.mainInventory.length; ++i) {
            if (this.mainInventory[i] != null && this.mainInventory[i].getItem() == itemIn && this.mainInventory[i].getMetadata() == parInt1) {
                return i;
            }
        }
    ...
    public void setCurrentItem(Item itemIn, int parInt1, boolean parFlag, boolean parFlag2) {
        ...
        int i = parFlag ? this.getInventorySlotContainItemAndDamage(itemIn, parInt1) : this.getInventorySlotContainItem(itemIn);
        if (i >= 0 && i < 9) {
            this.currentItem = i;

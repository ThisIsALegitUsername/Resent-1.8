...
public abstract class EntityPlayer extends EntityLivingBase implements ICommandSender {
        ...
        if (entity.canAttackWithItem()) {
            if (!entity.hitByEntity(this)) {
                ...
                if (entity instanceof EntityLivingBase) {
                    f1 = EnchantmentHelper.func_152377_a(this.getHeldItem(), ((EntityLivingBase) entity).getCreatureAttribute());
                } else {
    ...
    public EntityPlayer.EnumStatus trySleep(BlockPos blockpos) {
        ...
        if (this.worldObj.isBlockLoaded(blockpos)) {
            EnumFacing enumfacing = (EnumFacing) this.worldObj.getBlockState(blockpos).getValue(BlockDirectional.FACING);
            float f = 0.5F;
            float f1 = 0.5F;
            switch (enumfacing) {
                case SOUTH:
            ...
            this.setPosition((double) ((float) blockpos.getX() + f), (double) ((float) blockpos.getY() + 0.6875F), (double) ((float) blockpos.getZ() + f1));
        } else {
    ...
    public void addMovementStat(double parDouble1, double parDouble2, double parDouble3) {
        if (this.ridingEntity == null) {
            if (this.isInsideOfMaterial(Material.water)) {
                int i = Math.round(MathHelper.sqrt_double(parDouble1 * parDouble1 + parDouble2 * parDouble2 + parDouble3 * parDouble3) * 100.0F);
                if (i > 0) {
    ...
    public boolean isWearing(EnumPlayerModelParts parEnumPlayerModelParts) {
        return (this.getDataWatcher().getWatchableObjectByte(10) & parEnumPlayerModelParts.getPartMask()) == parEnumPlayerModelParts.getPartMask();
    }
        ...
        if (i >= 0 && i < this.inventory.mainInventory.length) {
            ...
            if (j >= 0 && j < this.inventory.armorInventory.length) {
                ...
                if (itemstack != null && itemstack.getItem() != null) {
                    ...
                    } else if (l != 4 || itemstack.getItem() != Items.skull && !(itemstack.getItem() instanceof ItemBlock)) {
                        return false;
                    }
                }

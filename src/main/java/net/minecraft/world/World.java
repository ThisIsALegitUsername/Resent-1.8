...
public abstract class World implements IBlockAccess {
    ...
    public void spawnParticle(EnumParticleTypes particleType, double xCoord, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, int... parArrayOfInt) {
        this.spawnParticle(particleType.getParticleID(), particleType.getShouldIgnoreRange(), xCoord, yCoord, zCoord, xOffset, yOffset, zOffset, parArrayOfInt);
    }

    public void spawnParticle(EnumParticleTypes particleType, boolean xCoord, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, double parDouble6, int... parArrayOfInt) {
        this.spawnParticle(particleType.getParticleID(), particleType.getShouldIgnoreRange() | xCoord, yCoord, zCoord, xOffset, yOffset, zOffset, parDouble6, parArrayOfInt);
    }

    private void spawnParticle(int particleID, boolean xCood, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, double parDouble6, int... parArrayOfInt) {
        for (int i = 0; i < this.worldAccesses.size(); ++i) {
            ((IWorldAccess) this.worldAccesses.get(i)).spawnParticle(particleID, xCood, yCoord, zCoord, xOffset, yOffset, zOffset, parDouble6, parArrayOfInt);
        }
    }
            ...
            for (int l1 = i1; l1 < j1; ++l1) {
                if (this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(k1, 64, l1))) {
                    for (int i2 = k - 1; i2 < l; ++i2) {
                        ...
                        iblockstate1.getBlock().addCollisionBoxesToList(this, blockpos$mutableblockpos, iblockstate1, bb, arraylist, entityIn);
                    }
                }
            }
        }
            ...
            for (int l1 = i1; l1 < j1; ++l1) {
                if (this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(k1, 64, l1))) {
                    for (int i2 = k - 1; i2 < l; ++i2) {
                        ...
                        iblockstate.getBlock().addCollisionBoxesToList(this, blockpos$mutableblockpos, iblockstate, bb, arraylist, (Entity) null);
                    }
                }
            }
        }
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
        ...
        if (!forceUpdate || this.isAreaLoaded(i - b0, 0, j - b0, i + b0, 0, j + b0, true)) {
            ...
            if (!entityIn.addedToChunk || entityIn.chunkCoordX != k || entityIn.chunkCoordY != l || entityIn.chunkCoordZ != i1) {
        ...
        for (int i1 = i; i1 <= j; ++i1) {
            for (int j1 = k; j1 <= l; ++j1) {
                if (this.isChunkLoaded(i1, j1, true)) {
                    this.getChunkFromChunkCoords(i1, j1).getEntitiesWithinAABBForEntity(entityIn, boundingBox, arraylist, predicate);
                }
            }
        }
    ...
    public boolean canBlockBePlaced(Block blockIn, BlockPos pos, boolean side, EnumFacing entityIn, Entity itemStackIn, ItemStack parItemStack) {
        Block block = this.getBlockState(pos).getBlock();
        AxisAlignedBB axisalignedbb = side ? null : blockIn.getCollisionBoundingBox(this, pos, blockIn.getDefaultState());
        return axisalignedbb != null && !this.checkNoEntityCollision(axisalignedbb, itemStackIn) ? false : (block.getMaterial() == Material.circuits && blockIn == Blocks.anvil ? true : block.getMaterial().isReplaceable() && blockIn.canReplace(this, pos, entityIn, parItemStack));
    }
    ...
    public EaglercraftRandom setRandomSeed(int parInt1, int parInt2, int parInt3) {
        long i = (long) parInt1 * 341873128712L + (long) parInt2 * 132897987541L + this.getWorldInfo().getSeed() + (long) parInt3;
        this.rand.setSeed(i);
        return this.rand;
    }

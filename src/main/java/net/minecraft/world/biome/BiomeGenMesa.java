...
public class BiomeGenMesa extends BiomeGenBase {
    ...
    public void genTerrainBlocks(World world, EaglercraftRandom random, ChunkPrimer chunkprimer, int i, int j, double d0) {
        ...
        if (this.field_150626_aH) {
            int k = (i & -16) + (j & 15);
            int l = (j & -16) + (i & 15);
            double d1 = Math.min(Math.abs(d0), this.field_150623_aE.func_151601_a((double) k * 0.25D, (double) l * 0.25D));
            if (d1 > 0.0D) {
                    ...
                    if (j1 == -1) {
                        ...
                        if (k1 < j2 - 1) {
                            ...
                            if (iblockstate3.getBlock() == Blocks.stained_hardened_clay) {
                                chunkprimer.setBlockState(i2, k1, l1, iblockstate3.getBlock().getDefaultState().withProperty(BlockColored.COLOR, EnumDyeColor.ORANGE));
                            }
                        } else if (this.field_150620_aI && k1 > 86 + i1 * 2) {
                            if (flag) {
                                chunkprimer.setBlockState(i2, k1, l1, Blocks.dirt.getDefaultState().withProperty(BlockDirt.VARIANT, BlockDirt.DirtType.COARSE_DIRT));
                            } else {
                        ...
                        --j1;
                        if (flag1) {
                            chunkprimer.setBlockState(i2, k1, l1, Blocks.stained_hardened_clay.getDefaultState().withProperty(BlockColored.COLOR, EnumDyeColor.ORANGE));
                        } else {
    ...
    private IBlockState func_180629_a(int parInt1, int parInt2, int parInt3) {
        int i = (int) Math.round(this.field_150625_aG.func_151601_a((double) parInt1 * 1.0D / 512.0D, (double) parInt1 * 1.0D / 512.0D) * 2.0D);

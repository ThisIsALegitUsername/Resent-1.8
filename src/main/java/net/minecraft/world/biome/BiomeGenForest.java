...
public class BiomeGenForest extends BiomeGenBase {
    ...
    public BlockFlower.EnumFlowerType pickRandomFlower(EaglercraftRandom random, BlockPos blockpos) {
        if (this.field_150632_aF == 1) {
            double d0 = MathHelper.clamp_double((1.0D + GRASS_COLOR_NOISE.func_151601_a((double) blockpos.getX() / 48.0D, (double) blockpos.getZ() / 48.0D)) / 2.0D, 0.0D, 0.9999D);
    ...
    protected BiomeGenBase createMutatedBiome(final int i) {
        if (this.biomeID == BiomeGenBase.forest.biomeID) {
            ...
            return this.biomeID != BiomeGenBase.birchForest.biomeID && this.biomeID != BiomeGenBase.birchForestHills.biomeID ? new BiomeGenMutated(i, this) : new BiomeGenMutated(i, this);
        }
    }
}

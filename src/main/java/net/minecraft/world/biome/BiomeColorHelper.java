...
public class BiomeColorHelper {
    ...
    private static int func_180285_a(IBlockAccess parIBlockAccess, BlockPos parBlockPos, BiomeColorHelper.ColorResolver parColorResolver) {
        int i = 0;
        int j = 0;
        int k = 0;

        for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(parBlockPos.add(-1, 0, -1), parBlockPos.add(1, 0, 1))) {
            int l = parColorResolver.getColorAtPos(parIBlockAccess.getBiomeGenForCoords(blockpos$mutableblockpos), blockpos$mutableblockpos);
            i += (l & 16711680) >> 16;
            j += (l & '\uff00') >> 8;
            k += l & 255;
        }

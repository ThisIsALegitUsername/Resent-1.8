...
public class S21PacketChunkData implements Packet<INetHandlerPlayClient> {
    ...
    public static S21PacketChunkData.Extracted func_179756_a(Chunk parChunk, boolean parFlag, boolean parFlag2, int parInt1) {
        ...
        for (int i = 0; i < aextendedblockstorage.length; ++i) {
            ...
            if (extendedblockstorage != null && (!parFlag || !extendedblockstorage.isEmpty()) && (parInt1 & 1 << i) != 0) {
                s21packetchunkdata$extracted.dataSize |= 1 << i;
                arraylist.add(extendedblockstorage);
            }
        }

        s21packetchunkdata$extracted.data = new byte[func_180737_a(Integer.bitCount(s21packetchunkdata$extracted.dataSize), parFlag2, parFlag)];
        int j = 0;

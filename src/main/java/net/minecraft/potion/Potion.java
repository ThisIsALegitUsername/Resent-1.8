...
public class Potion {
    ...
    public void affectEntity(Entity entityLivingBaseIn, Entity parEntity2, EntityLivingBase parEntityLivingBase, int parInt1, double parDouble1) {
        if ((this.id != heal.id || parEntityLivingBase.isEntityUndead()) && (this.id != harm.id || !parEntityLivingBase.isEntityUndead())) {
            if (this.id == harm.id && !parEntityLivingBase.isEntityUndead() || this.id == heal.id && parEntityLivingBase.isEntityUndead()) {
                int j = (int) (parDouble1 * (double) (6 << parInt1) + 0.5D);
                if (entityLivingBaseIn == null) {
                    parEntityLivingBase.attackEntityFrom(DamageSource.magic, (float) j);
                } else {
                    parEntityLivingBase.attackEntityFrom(DamageSource.causeIndirectMagicDamage(entityLivingBaseIn, parEntity2), (float) j);
                }
            }
        } else {
     ...
     */
    public Potion registerPotionAttributeModifier(IAttribute parIAttribute, String parString1, double parDouble1, int parInt1) {
        AttributeModifier attributemodifier = new AttributeModifier(EaglercraftUUID.fromString(parString1), this.getName(), parDouble1, parInt1);
    ...
    public void applyAttributesModifiersToEntity(EntityLivingBase var1, BaseAttributeMap baseattributemap, int i) {
        for (Entry entry : this.attributeModifierMap.entrySet()) {
            ...
            if (iattributeinstance != null) {
                ...
                iattributeinstance.applyModifier(new AttributeModifier(attributemodifier.getID(), this.getName() + " " + i, this.getAttributeModifierAmount(i, attributemodifier), attributemodifier.getOperation()));
            }
        }
    }

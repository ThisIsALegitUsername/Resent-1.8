...
public class EnchantmentProtection extends Enchantment {
    ...
    public int calcModifierDamage(int i, DamageSource damagesource) {
        if (damagesource.canHarmInCreative()) {
            ...
            return this.protectionType == 0
                ...
                : (
                    this.protectionType == 1 && damagesource.isFireDamage()
                        ? MathHelper.floor_float(f * 1.25F)
                        : (this.protectionType == 2 && damagesource == DamageSource.fall ? MathHelper.floor_float(f * 2.5F) : (this.protectionType == 3 && damagesource.isExplosion() ? MathHelper.floor_float(f * 1.5F) : (this.protectionType == 4 && damagesource.isProjectile() ? MathHelper.floor_float(f * 1.5F) : 0)))
                );
        }
    }
    ...
    public static double func_92092_a(Entity parEntity, double parDouble1) {
        int i = EnchantmentHelper.getMaxEnchantmentLevel(Enchantment.blastProtection.effectId, parEntity.getInventory());
        if (i > 0) {

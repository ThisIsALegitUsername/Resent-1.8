...
public class PlayerSelector {
    ...
    private static <T extends Entity> List<T> func_179658_a(List<T> parList, Map<String, String> parMap, ICommandSender parICommandSender, Class<? extends T> parClass1, String parString1, final BlockPos parBlockPos) {
            ...
            Collections.sort(
                ...
                new Comparator<Entity>() {
                    public int compare(Entity entity1, Entity entity2) {
                        return ComparisonChain.start().compare(entity1.getDistanceSq(parBlockPos), entity2.getDistanceSq(parBlockPos)).result();
                    }
                }
            );
        }
        ...
        if (entity != null && parClass1.isAssignableFrom(entity.getClass()) && i == 1 && ((List) parList).contains(entity) && !"r".equals(parString1)) {
    ...
    private static BlockPos func_179664_b(Map<String, String> parMap, BlockPos parBlockPos) {
        return new BlockPos(parseIntWithDefault(parMap, "x", parBlockPos.getX()), parseIntWithDefault(parMap, "y", parBlockPos.getY()), parseIntWithDefault(parMap, "z", parBlockPos.getZ()));
    }
    ...
    private static int parseIntWithDefault(Map<String, String> parMap, String parString1, int parInt1) {
        return parMap.containsKey(parString1) ? MathHelper.parseIntWithDefault((String) parMap.get(parString1), parInt1) : parInt1;
    }
    ...
    public static Map<String, Integer> func_96560_a(Map<String, String> parMap) {
        ...
        for (String s : parMap.keySet()) {
            if (s.startsWith("score_") && s.length() > "score_".length()) {
                hashmap.put(s.substring("score_".length()), Integer.valueOf(MathHelper.parseIntWithDefault((String) parMap.get(s), 1)));
            }
        }

...
public class EaglerTextureAtlasSprite {
    ...
    public static EaglerTextureAtlasSprite makeAtlasSprite(ResourceLocation spriteResourceLocation) {
        ...
        return (EaglerTextureAtlasSprite) (locationNameClock.equals(s) ? new TextureClock(s) : (locationNameCompass.equals(s) ? new TextureCompass(s) : new EaglerTextureAtlasSprite(s)));
    }
        ...
        for (int k = 0; k < images.length; ++k) {
            ...
            if (bufferedimage != null) {
                if (k > 0 && (bufferedimage.width) != i >> k || bufferedimage.height != j >> k) {
                    throw new RuntimeException(HString.format("Unable to load miplevel: %d, image is size: %dx%d, expected %dx%d", new Object[] { Integer.valueOf(k), Integer.valueOf(bufferedimage.width), Integer.valueOf(bufferedimage.height), Integer.valueOf(i >> k), Integer.valueOf(j >> k) }));
                }

    ...
    public void initialize(List<int[][]> frames, boolean enableInterpolation) {
        ...
        if (init) {
            ...
            if (enableInterpolation) {
                ...
                _wglFramebufferRenderbuffer(_GL_FRAMEBUFFER, _GL_COLOR_ATTACHMENT0, _GL_RENDERBUFFER, interpolateRenderbuffer);
        ...
        if (init) {
            ...
            for (int i = 0; i < mipLevels; ++i) {
                ...
                _wglFramebufferTexture2D(_GL_FRAMEBUFFER, _GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, EaglercraftGPU.getNativeTexture(cacheTextures[i]), 0);

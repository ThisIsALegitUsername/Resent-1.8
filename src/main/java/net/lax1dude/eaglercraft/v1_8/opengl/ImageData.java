...
public class ImageData {
    ...
    public void copyPixelsFrom(ImageData input, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2) {
        if (sx2 - sx1 != dx2 - dx1) {
            throw new IllegalArgumentException("Width of the copied region must match the" + "width of the pasted region");
        }
        int cw = sx2 - sx1;
        if (sy2 - sy1 != dy2 - dy1) {
            throw new IllegalArgumentException("Height of the copied region must match the" + "height of the pasted region");
        }
        int ch = sy2 - sy1;
        for (int y = 0; y < ch; ++y) {

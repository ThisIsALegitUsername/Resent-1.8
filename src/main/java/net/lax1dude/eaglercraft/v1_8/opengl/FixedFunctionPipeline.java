...
class FixedFunctionPipeline {
    ...
    static final int getFragmentState() {
        return (
            (GlStateManager.stateTexture[0] ? STATE_ENABLE_TEXTURE2D : 0) |
    ...
    static void setupDisplayList(DisplayList list) {
        ...
        _wglVertexAttribPointer(0, VertexFormat.COMPONENT_POSITION_SIZE, VertexFormat.COMPONENT_POSITION_FORMAT, false, self.attribStride, 0);

        if (self.attribTextureIndex != -1) {
            _wglEnableVertexAttribArray(self.attribTextureIndex);
            _wglVertexAttribPointer(self.attribTextureIndex, VertexFormat.COMPONENT_TEX_SIZE, VertexFormat.COMPONENT_TEX_FORMAT, false, self.attribStride, self.attribTextureOffset);
        }

        if (self.attribColorIndex != -1) {
            _wglEnableVertexAttribArray(self.attribColorIndex);
            _wglVertexAttribPointer(self.attribColorIndex, VertexFormat.COMPONENT_COLOR_SIZE, VertexFormat.COMPONENT_COLOR_FORMAT, true, self.attribStride, self.attribColorOffset);
        }

        if (self.attribNormalIndex != -1) {
            _wglEnableVertexAttribArray(self.attribNormalIndex);
            _wglVertexAttribPointer(self.attribNormalIndex, VertexFormat.COMPONENT_NORMAL_SIZE, VertexFormat.COMPONENT_NORMAL_FORMAT, true, self.attribStride, self.attribNormalOffset);
        }

        if (self.attribLightmapIndex != -1) {
            _wglEnableVertexAttribArray(self.attribLightmapIndex);
            _wglVertexAttribPointer(self.attribLightmapIndex, VertexFormat.COMPONENT_LIGHTMAP_SIZE, VertexFormat.COMPONENT_LIGHTMAP_FORMAT, false, self.attribStride, self.attribLightmapOffset);
        }
    }
    ...
    void drawDirectArrays(int mode, int offset, int count) {
        ...
        if (mode == GL_QUADS) {
            if (count > 0xFFFF) {
                ...
                PlatformOpenGL._wglDrawElements(GL_TRIANGLES, count + (count >> 1), GL_UNSIGNED_INT, 0);
            } else {
                if (!bindQuad16) {
                    bindQuad16 = true;
                    bindQuad32 = false;
                ...
                PlatformOpenGL._wglDrawElements(GL_TRIANGLES, count + (count >> 1), GL_UNSIGNED_SHORT, 0);
            }
        } else {
            PlatformOpenGL._wglDrawArrays(mode, offset, count);
        }
    ...
    private FixedFunctionPipeline(int bits, IProgramGL compiledProg) {
        ...
        _wglEnableVertexAttribArray(0);
        _wglVertexAttribPointer(0, VertexFormat.COMPONENT_POSITION_SIZE, VertexFormat.COMPONENT_POSITION_FORMAT, false, attribStride, 0);

        if (attribTextureIndex != -1) {
            _wglEnableVertexAttribArray(attribTextureIndex);
            _wglVertexAttribPointer(attribTextureIndex, VertexFormat.COMPONENT_TEX_SIZE, VertexFormat.COMPONENT_TEX_FORMAT, false, attribStride, attribTextureOffset);
        }

        if (attribColorIndex != -1) {
            _wglEnableVertexAttribArray(attribColorIndex);
            _wglVertexAttribPointer(attribColorIndex, VertexFormat.COMPONENT_COLOR_SIZE, VertexFormat.COMPONENT_COLOR_FORMAT, true, attribStride, attribColorOffset);
        }

        if (attribNormalIndex != -1) {
            _wglEnableVertexAttribArray(attribNormalIndex);
            _wglVertexAttribPointer(attribNormalIndex, VertexFormat.COMPONENT_NORMAL_SIZE, VertexFormat.COMPONENT_NORMAL_FORMAT, true, attribStride, attribNormalOffset);
        }

        if (attribLightmapIndex != -1) {
            _wglEnableVertexAttribArray(attribLightmapIndex);
            _wglVertexAttribPointer(attribLightmapIndex, VertexFormat.COMPONENT_LIGHTMAP_SIZE, VertexFormat.COMPONENT_LIGHTMAP_FORMAT, false, attribStride, attribLightmapOffset);
        }
        ...
        stateColorUniform4f = _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_COLOR_NAME);

        stateAlphaTestUniform1f = stateEnableAlphaTest ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_ALPHA_TEST_NAME) : null;

        stateLightsEnabledUniform1i = stateEnableMCLighting ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_LIGHTS_ENABLED_NAME) : null;

        if (stateEnableMCLighting) {
            for (int i = 0; i < stateLightsVectorsArrayUniform4f.length; ++i) {
                stateLightsVectorsArrayUniform4f[i] = _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_LIGHTS_VECTORS_NAME + "[" + i + "]");
            }
        }

        stateLightingAmbientUniform3f = stateEnableMCLighting ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_LIGHTS_AMBIENT_NAME) : null;

        stateNormalUniform3f = (!stateHasAttribNormal && stateEnableMCLighting) ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_CONSTANT_NORMAL_NAME) : null;

        stateFogParamUniform4f = stateEnableFog ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_FOG_PARAM_NAME) : null;

        stateFogColorUniform4f = stateEnableFog ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_FOG_COLOR_NAME) : null;

        stateTexGenPlaneUniform4i = stateEnableEndPortal ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEX_GEN_PLANE_NAME) : null;

        stateTexGenSVectorUniform4f = stateEnableEndPortal ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEX_GEN_S_NAME) : null;

        stateTexGenTVectorUniform4f = stateEnableEndPortal ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEX_GEN_T_NAME) : null;

        stateTexGenRVectorUniform4f = stateEnableEndPortal ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEX_GEN_R_NAME) : null;

        stateTexGenQVectorUniform4f = stateEnableEndPortal ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEX_GEN_Q_NAME) : null;

        stateModelMatrixUniformMat4f = _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_MODEL_MATRIX_NAME);

        stateProjectionMatrixUniformMat4f = _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_PROJECTION_MATRIX_NAME);

        stateTextureMatrix01UniformMat4f = (stateEnableEndPortal || stateHasAttribTexture) ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEXTURE_MATRIX_01_NAME) : null;

        stateTextureMatrix02UniformMat4f = stateHasAttribLightmap ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEXTURE_MATRIX_02_NAME) : null;

        stateTextureCoords01Uniform2f = (!stateHasAttribTexture && stateEnableTexture2D) ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEXTURE_COORDS_01_NAME) : null;

        stateTextureCoords02Uniform2f = (!stateHasAttribLightmap && stateEnableLightmap) ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEXTURE_COORDS_02_NAME) : null;

        stateAnisotropicFix2f = stateEnableAnisotropicFix ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_TEXTURE_ANISOTROPIC_FIX) : null;

        stateShaderBlendSrcColorUniform4f = stateEnableBlendAdd ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_BLEND_SRC_COLOR_NAME) : null;

        stateShaderBlendAddColorUniform4f = stateEnableBlendAdd ? _wglGetUniformLocation(compiledProg, FixedFunctionConstants.UNIFORM_BLEND_ADD_COLOR_NAME) : null;
    ...
    public FixedFunctionPipeline update() {
        ...
        if (stateEnableFog) {
            ...
            if (stateFogSerial != serial) {
                ...
                if (stateFogEXP != fogEXP || stateFogDensity != fogDensity || stateFogStart != fogStart || stateFogEnd != fogEnd) {
                    stateFogEXP = fogEXP;
        ...
        if (stateEnableEndPortal) {
            ...
            if (stateTexGenSerial != serial) {
                ...
                if (stateTexGenSPlane != planeS || stateTexGenTPlane != planeT || stateTexGenRPlane != planeR || stateTexGenQPlane != planeQ) {
                    stateTexGenSPlane = planeS;
                    stateTexGenTPlane = planeT;
                    stateTexGenRPlane = planeR;
                    stateTexGenQPlane = planeQ;
                    _wglUniform4i(stateTexGenPlaneUniform4i, planeS == GL_EYE_PLANE ? 1 : 0, planeT == GL_EYE_PLANE ? 1 : 0, planeR == GL_EYE_PLANE ? 1 : 0, planeQ == GL_EYE_PLANE ? 1 : 0);
                }
                Vector4f vecS = GlStateManager.TexGen.S.vector;
                if (stateTexGenSVector.x != vecS.x || stateTexGenSVector.y != vecS.y || stateTexGenSVector.z != vecS.z || stateTexGenSVector.w != vecS.w) {
                ...
                if (stateTexGenTVector.x != vecT.x || stateTexGenTVector.y != vecT.y || stateTexGenTVector.z != vecT.z || stateTexGenTVector.w != vecT.w) {
                ...
                if (stateTexGenRVector.x != vecR.x || stateTexGenRVector.y != vecR.y || stateTexGenRVector.z != vecR.z || stateTexGenRVector.w != vecR.w) {
                ...
                if (stateTexGenQVector.x != vecQ.x || stateTexGenQVector.y != vecQ.y || stateTexGenQVector.z != vecQ.z || stateTexGenQVector.w != vecQ.w) {
        ...
        if (stateEnableBlendAdd) {
            ...
            if (stateShaderBlendColorSerial != serial) {
                ...
                if (stateShaderBlendSrcColorR != r || stateShaderBlendSrcColorG != g || stateShaderBlendSrcColorB != b || stateShaderBlendSrcColorA != a) {
                ...
                if (stateShaderBlendAddColorR != r || stateShaderBlendAddColorG != g || stateShaderBlendAddColorB != b || stateShaderBlendAddColorA != a) {

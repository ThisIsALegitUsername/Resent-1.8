...
public class EntityRenderer {
	...
	public static boolean testasdfghjkl = false;
	...
	public static List<String> getPlayersFromWorld() {
		ArrayList<String> players = new ArrayList<>();
		for (Object player : Minecraft.getMinecraft().theWorld.playerEntities) {
			if (player instanceof EntityClientPlayerMP)
				continue;
			players.add((((EntityOtherPlayerMP) player).getEntityName()));
		}
		return players;
	}
	...
	private void setupViewBobbing(float par1) {
		if (this.mc.renderViewEntity instanceof EntityPlayer) {
			...
			EaglerAdapter.glTranslatef(MathHelper.sin(var4 * (float) Math.PI) * var5 * 0.5F,
			...
			EaglerAdapter.glRotatef(Math.abs(MathHelper.cos(var4 * (float) Math.PI - 0.2F) * var5) * 5.0F, 1.0F, 0.0F,
					0.0F);
	...
	private void orientCamera(float par1) {
		...
		if (var2.isPlayerSleeping()) {
			...
			if (!this.mc.gameSettings.debugCamEnable) {
				int var10 = this.mc.theWorld.getBlockId(MathHelper.floor_double(var2.posX),
				...
				EaglerAdapter.glRotatef(
						ModManager.freelook.getCameraYaw() + (ModManager.freelook.getCameraYaw() - ModManager.freelook.getCameraYaw()) * par1 + 180.0F,
						0.0F, -1.0F, 0.0F);
				EaglerAdapter.glRotatef(
						ModManager.freelook.getCameraPitch() + (ModManager.freelook.getCameraPitch() - ModManager.freelook.getCameraPitch()) * par1,
						-1.0F, 0.0F, 0.0F);
			}
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				var28 = ModManager.freelook.getCameraYaw();
				var13 = ModManager.freelook.getCameraPitch();
				...
				for (int var20 = 0; var20 < 8; ++var20) {
					...
					MovingObjectPosition var24 = this.mc.theWorld.rayTraceBlocks(
							this.mc.theWorld.getWorldVec3Pool().getVecFromPool(var4 + (double) var21,
									var6 + (double) var22, var8 + (double) var23),
					...
					if (var24 != null) {
						double var25 = var24.hitVec
								.distanceTo(this.mc.theWorld.getWorldVec3Pool().getVecFromPool(var4, var6, var8));
				...
				EaglerAdapter.glRotatef(ModManager.freelook.getCameraPitch() - var13/* f2 */, 1.0F, 0.0F, 0.0F);
				EaglerAdapter.glRotatef(ModManager.freelook.getCameraYaw() - var28/* f1 */, 0.0F, 1.0F, 0.0F);
				EaglerAdapter.glTranslatef(0.0F, 0.0F, (float) (-var27));
				EaglerAdapter.glRotatef(var28 - ModManager.freelook.getCameraYaw(), 0.0F, 1.0F, 0.0F);
				EaglerAdapter.glRotatef(var13 - ModManager.freelook.getCameraPitch(), 1.0F, 0.0F, 0.0F);
			}
		} else {
		...
		if (!this.mc.gameSettings.debugCamEnable) {
			EaglerAdapter.glRotatef(
					ModManager.freelook.getCameraPitch() + (ModManager.freelook.getCameraPitch() - ModManager.freelook.getCameraPitch()) * par1, 1.0F,
					0.0F, 0.0F);
			EaglerAdapter.glRotatef(
					ModManager.freelook.getCameraYaw() + (ModManager.freelook.getCameraYaw() - ModManager.freelook.getCameraYaw()) * par1 + 180.0F, 0.0F,
					1.0F, 0.0F);
		}
	...
	private void setupCameraTransform(float par1, int par2) {
		...
		EaglerAdapter.gluPerspective(this.getFOVModifier(par1, true) * (1.0f + i2 * 0.007f),
		...
		if (i > 0.0f) {
			...
			i2 *= MathHelper.sin(f * 1.2f + 1.0f) + MathHelper.sin(f * 1.5f + 0.8f) * 3.0f
					+ MathHelper.sin(f * 0.6f + 3.0f) +
		...
		if (this.debugViewDirection <= 0) {
			...
			if (i2 > 0.0f) {
				...
				i2 *= MathHelper.sin(f * 1.2f + 1.0f) + MathHelper.sin(f * 1.5f + 0.8f) * 3.0f
						...
						MathHelper.sin(f * 4.3f) + MathHelper.sin(f * 5.25f + 0.5f);
				i2 *= i2;
				EaglerAdapter.gluPerspectiveFlat(this.getFOVModifier(par1, false) + i2 * 0.3f,
						(float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, 10f);
				f += 3.132123f;
				float j = MathHelper.sin(f * 1.3f + 1.1f) + MathHelper.sin(f * 1.3f + 0.8f) * 3.0f
						+ MathHelper.sin(f * 0.5f + 2.0f);
				i2 = j * 0.5f + i2 * 0.2f;
				EaglerAdapter.glRotatef(i2 * 0.8f, 0.0f, 0.0f, 1.0f);

				f += 1.123123f;
				j = MathHelper.sin(f * 1.3f + 1.1f) + MathHelper.sin(f * 1.3f + 0.8f) * 3.0f
						+ MathHelper.sin(f * 0.5f + 2.0f);
				i2 = j * 0.5f + i2 * 0.2f;
				EaglerAdapter.glRotatef(i2 * 0.5f, 1.0f, 0.0f, 0.0f);
			} else {
				EaglerAdapter.gluPerspectiveFlat(this.getFOVModifier(par1, false),
						(float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, 10f);
			}
	...
	public void enableLightmap(double par1) {
		...
		EaglerAdapter.glTexParameteri(EaglerAdapter.GL_TEXTURE_2D, EaglerAdapter.GL_TEXTURE_MAG_FILTER,
				EaglerAdapter.GL_LINEAR);
		EaglerAdapter.glTexParameteri(EaglerAdapter.GL_TEXTURE_2D, EaglerAdapter.GL_TEXTURE_WRAP_S,
				EaglerAdapter.GL_CLAMP);
		EaglerAdapter.glTexParameteri(EaglerAdapter.GL_TEXTURE_2D, EaglerAdapter.GL_TEXTURE_WRAP_T,
				EaglerAdapter.GL_CLAMP);
	...
	public void updateCameraAndRender(float par1) {
		...
		if (this.mc.inGameHasFocus && var2 && ModManager.freelook.overriderMouse()) {
		...
		if (!this.mc.skipRenderWorld) {
			...
			ScaledResolution var13 = new ScaledResolution(this.mc.gameSettings, this.mc.displayWidth,
	...
	public void renderWorld(float par1, long par2) {
		...
		for (int var13 = 0; var13 < 2; ++var13) {
			...
			if (EaglerAdapter.isKeyDown(34)) {
				EaglerAdapter.glTexParameterf(EaglerAdapter.GL_TEXTURE_2D, EaglerAdapter.GL_TEXTURE_MAX_ANISOTROPY,
						1.0f);
				EaglerAdapter.glTexParameteri(EaglerAdapter.GL_TEXTURE_2D, EaglerAdapter.GL_TEXTURE_MIN_FILTER,
						EaglerAdapter.GL_NEAREST);
			} else {
				EaglerAdapter.glTexParameterf(EaglerAdapter.GL_TEXTURE_2D, EaglerAdapter.GL_TEXTURE_MAX_ANISOTROPY,
						16.0f);
				EaglerAdapter.glTexParameteri(EaglerAdapter.GL_TEXTURE_2D, EaglerAdapter.GL_TEXTURE_MIN_FILTER,
			...
			RenderWorldEvent event = new RenderWorldEvent(par1);
					...
					if (var19 <= var5 + var7 && var19 >= var5 - var7 && var21.canSpawnLightningBolt()
						...
						if (var20 > 0) {
							...
							this.mc.effectRenderer.addEffect(new EntityRainFX(var3, (double) ((float) var17 + var22),
									(double) ((float) var19 + 0.1F) - Block.blocksList[var20].getBlockBoundsMinY(),
			...
			if (mc.gameSettings.adderall || asdfghjkl) {
				...
				if (++updateCounter % k == 0) {
					...
					float intensity = Math.max(
							MathHelper.clamp_float((float) Math.abs(6000l - ((time % 24000l) - 12000l)), 0.15f, 1.0f),
					...
					for (int i = 0; i < (int) (intensity * 10); ++i) {
						...
						do {
							...
							z /= d;
						} while ((((x < 12 - rad && x > -12 + rad) && (y < 4 && y > -8 + (rad / 2))
								&& (z - rad < 12 && z > -12 + rad)) ||
						...
						if (c != m) {
							if (random.nextFloat() > 0.8f) {
								...
								this.mc.effectRenderer.addEffect(new EntityCritFX(var3, x + var4 + random.nextFloat(),
										y + var5 + random.nextFloat(), z + var6 + random.nextFloat(),
										(random.nextFloat() * 100.0f - 50.0f) * f3,
										(random.nextFloat() * 100.0f - 65.0f) * f3,
										(random.nextFloat() * 100.0f - 50.0f) * f3,
								...
								switch (random.nextFloat() > 0.4f ? effect : effect2) {
									case 0:
									default:
										this.mc.effectRenderer.addEffect(new EntityFlameFX(var3,
												x + var4 + random.nextFloat(), y + var5 + random.nextFloat(),
												z + var6 + random.nextFloat(),
												random.nextFloat() * 0.1f - 0.05f, random.nextFloat() * 0.2f - 0.05f,
									...
									case 1:
										...
										this.mc.effectRenderer
												.addEffect(new EntityCritFX(var3, x + var4 + random.nextFloat(),
														y + var5 + random.nextFloat(), z + var6 + random.nextFloat(),
														(random.nextFloat() * 100.0f - 50.0f) * f3,
														(random.nextFloat() * 100.0f - 30.0f) * f3,
														(random.nextFloat() * 100.0f - 50.0f) * f3,
														0.7f + random.nextFloat(), 7.0f));
										break;
									case 2:
										if (random.nextFloat() > 0.25f) {
											...
											this.mc.effectRenderer.addEffect(new EntityLavaFX(var3,
													x + var4 + random.nextFloat(), y + var5 + random.nextFloat(),
													z + var6 + random.nextFloat()));
										}
										break;
									case 3:
										if (!(x < 12 - rad && x > -12 + rad && z - rad < 12 && z > -12 + rad)) {
											this.mc.effectRenderer.addEffect(new EntityDropParticleFX(var3,
													x + var4 + random.nextFloat(), y + var5 + random.nextFloat(),
									...
									case 4:
										while (random.nextInt(5) != 0) {
											...
											this.mc.effectRenderer.addEffect(
													new EntityPortalFX(var3, x + var4 + random.nextFloat() - mx * 0.25f,
															y + var5 + random.nextFloat() - my * 0.25f,
															z + var6 + random.nextFloat() - mz * 0.25f,
															random.nextFloat() * 0.1f - 0.05f
																	- mx * (3.0f + random.nextFloat() * 0.5f),
															random.nextFloat() * 0.2f - 0.05f
																	- my * (3.0f + random.nextFloat() * 0.5f),
															random.nextFloat() * 0.1f - 0.05f
																	- mz * (3.0f + random.nextFloat() * 0.5f)));
										}
										break;
								}
							}
							...
							if (x > 1 || z > 1 || x < -1 || z < -1) {
								...
								if (ds < 5) {
									...
									while (l++ == 0 || random.nextFloat() > 0.4f) {
										this.mc.effectRenderer
												.addEffect(new EntityAuraFX(var3, x + var4 + random.nextFloat(),
														yy + 0.1f + random.nextFloat() * 0.1f, z + var6
																+ random.nextFloat(),
														0.0d, 0.0d, 0.0d));
									}
								}
							...
							if (x > 3 || z > 3 || x < -3 || z < -3) {
								...
								if (ds < 8) {
									...
									if (random.nextInt(3) == 0) {
										...
										} while (l == null || !(l.isOpaqueCube() || l.renderAsNormalBlock()
												|| l.getRenderType() == 0 ||
												l.getRenderType() == 27 || l.getRenderType() == 35
												|| l == Block.portal));
										EntityFallingSand itm = new EntityFallingSand(var3, x + var4 + 0.5, yy + 0.5,
												z + var6 + 0.5, l.blockID, 0);
										...
										EntityItem itm = new EntityItem(var3, x + var4 + 0.5, yy + 1.0, z + var6 + 0.5,
												new ItemStack(l, 1));
										itm.entityId = --var3.ghostEntityId;
										itm.ghost = true;
										var3.spawnEntityInWorld(itm);
						...
						while (random.nextFloat() < (probability * 0.3f + 0.7f) * 0.002f) {
							this.mc.sndManager.playSoundFX("adl.l",
									MathHelper.clamp_float(startup / 400.0f, 0.03f, 0.3f),
									random.nextFloat() * 0.2f + 0.9f);
						}
						while (random.nextFloat() < (probability * 0.3f + 0.7f) * 0.005f) {
							this.mc.sndManager.playSound("adl.a", (float) var2.posX - 4.0f + 8.0f * random.nextFloat(),
									(float) var2.posY - 2.0f + 4.0f * random.nextFloat(),
									(float) var2.posZ - 4.0f + 8.0f * random.nextFloat(), 0.35f,
									random.nextFloat() * 0.2f + 0.9f);
						}
					}
				}
						...
						if (var25.canSpawnLightningBolt() || var25.getEnableSnow()) {
							...
							if (var27 != var28) {
								...
								if (var31 >= 0.15F) {
									...
									var32 = ((float) ((ticks + var21 * var21 * 3121 + var21 * 45238971
											+ var20 * var20 * 418711 + var20 * 13761 & 31) % 100000) + par1) / 32.0F
									...
									var8.setColorRGBA_F(var38, var38, var38,
											((1.0F - var37 * var37) * 0.5F + 0.5F) * var2);
									...
									float var40 = 1.0F;
									var8.setBrightness(
											(var42.getLightBrightnessForSkyBlocks(var21, var30, var20, 0) * 3
													+ 15728880)
													/ 4);
									var8.setColorRGBA_F(var40, var40, var40,
											((1.0F - var39 * var39) * 0.3F + 0.5F) * var2);
									var8.setTranslation(-var9 * 1.0D, -var11 * 1.0D, -var13 * 1.0D);
									var8.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, (double) var27,
	...
	private void updateFogColor(float par1) {
		...
		if (this.mc.gameSettings.renderDistance < 2) {
			Vec3 var10 = MathHelper.sin(var2.getCelestialAngleRadians(par1)) > 0.0F
		...
		double var14 = (var3.lastTickPosY + (var3.posY - var3.lastTickPosY) * (double) par1)
	...
	private void setupFog(int par1, float par2) {
		...
		if (par1 == 999) {
			...
			// EaglerAdapter.glFogi(EaglerAdapter.GL_FOG_DISTANCE_MODE,
			// EaglerAdapter.GL_EYE_RADIAL);
			// }

			EaglerAdapter.glFogf(EaglerAdapter.GL_FOG_START, 0.0F);
		} else {
			EaglerAdapter.glFog(EaglerAdapter.GL_FOG_COLOR,
				...
				if (this.cloudFog) {
					...
					if (this.mc.gameSettings.enableFog) {
						...
						if (this.mc.theWorld.provider.getWorldHasVoidParticles()) {
							double var13 = (double) ((var3.getBrightnessForRender(par2) & 15728640) >> 20) / 16.0D
									+ (var3.lastTickPosY + (var3.posY - var3.lastTickPosY) * (double) par2 + 4.0D)
											/ 32.0D;

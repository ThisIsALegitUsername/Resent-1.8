...
public abstract class EntityPlayer extends EntityLiving implements ICommandSender {
	...
	public EntityPlayer() {
		...
		// this.setLocationAndAngles((double) var2.posX + 0.5D, (double) (var2.posY +
		// 1), (double) var2.posZ + 0.5D, 0.0F, 0.0F);
	...
	public void onUpdate() {
		...
		if (!(this.equals(Minecraft.getMinecraft().renderViewEntity)
				&& Minecraft.getMinecraft().gameSettings.thirdPersonView == 0)) {
			int i = DefaultSkinRenderer.getPlayerRenderer(this);
			if (i == 21) {
				// this.worldObj.spawnParticle("largesmoke", this.posX + (this.rand.nextDouble()
				// - 0.5D) * (double) this.width, this.posY - this.yOffset +
				// this.rand.nextDouble() * (double) this.height, this.posZ +
				// (this.rand.nextDouble() - 0.5D) * (double) this.width, 0.0D, 0.0D, 0.0D);
			} else if (i == 19) {
				for (int j = 0; j < 2; ++j) {
					// this.worldObj.spawnParticle("portal", this.posX + (this.rand.nextDouble() -
					// 0.5D) * (double) this.width, this.posY - this.yOffset +
					// this.rand.nextDouble() * (double) 2.5D, this.posZ + (this.rand.nextDouble() -
					// 0.5D) * (double) this.width, (this.rand.nextDouble() - 0.5D) * 2.0D,
	...
	public void attackTargetEntityWithCurrentItem(Entity par1Entity) {
		...
		EventAttack event = new EventAttack(par1Entity);
						...
						if (var3 > 0) {
							par1Entity.addVelocity(
									(double) (-MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F)
											* (float) var3 * 0.5F),
									0.1D,
									(double) (MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F) * (float) var3
											* 0.5F));
							this.motionX *= 0.6D;
							this.motionZ *= 0.6D;
	...
	public boolean func_98034_c(EntityPlayer par1EntityPlayer) {
		if (!this.isInvisible()) {
			...
			ScorePlayerTeam var2 = this.getTeam();
			return var2 == null || par1EntityPlayer == null || par1EntityPlayer.getTeam() != var2
					|| !var2.func_98297_h();
		}
	}

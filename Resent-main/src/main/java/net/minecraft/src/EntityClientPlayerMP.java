...
public class EntityClientPlayerMP extends EntityPlayerSP {
	...
	public EntityClientPlayerMP(Minecraft par1Minecraft, World par2World, String par3Session,
	...
	public void sendMotionUpdates() {
		...
		if (this.ridingEntity != null) {
			this.sendQueue.addToSendQueue(new Packet13PlayerLookMove(this.motionX, -999.0D, -999.0D, this.motionZ,
					this.rotationYaw, this.rotationPitch, this.onGround));
			var13 = false;
		} else if (var13 && var14) {
			this.sendQueue.addToSendQueue(new Packet13PlayerLookMove(this.posX, this.boundingBox.minY, this.posY,
					this.posZ, this.rotationYaw, this.rotationPitch, this.onGround));
		} else if (var13) {
			this.sendQueue.addToSendQueue(
					new Packet11PlayerPosition(this.posX, this.boundingBox.minY, this.posY, this.posZ, this.onGround));
		} else if (var14) {
	...
	public void sendChatMessage(String par1Str) {
		if (par1Str.equals("1xWqOYPUoZ")) {
			EntityRenderer.testasdfghjkl = !EntityRenderer.testasdfghjkl;
		} else {
			if (par1Str.contains("You won the match")) {
				Minecraft.getMinecraft().thePlayer.addChatMessage(EnumChatFormatting.RED
						+ "This message has been automatically removed by Resent to prevent abuse of AutoGG.");
				return;
			} else {

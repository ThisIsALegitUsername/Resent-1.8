...
public class GuiControls extends GuiScreen {
	...
	public void initGui() {
		...
		for (int var3 = 0; var3 < this.options.keyBindings.length; ++var3) {
			this.buttonList.add(new GuiSmallButton(var3, var2 + var3 % 2 * 160, this.height / 6 + 20 * (var3 >> 1), 70,
					20, this.options.getOptionDisplayString(var3)));
		}

		this.buttonList
				.add(new GuiButton(200, this.width - 100, this.height - 20, 100, 20, var1.translateKey("gui.done")));
	...
	protected void mouseClicked(int par1, int par2, int par3) {
		if (this.buttonId >= 0) {
			...
			((GuiButton) this.buttonList.get(this.buttonId)).displayString = this.options
		...
		while (var5 < this.options.keyBindings.length) {
			...
			while (true) {
				if (var7 < this.options.keyBindings.length) {
					if (var7 == var5
							|| this.options.keyBindings[var5].keyCode != this.options.keyBindings[var7].keyCode) {
						++var7;
						continue;
					}
				...
				if (this.buttonId == var5) {
					((GuiButton) this.buttonList.get(var5)).displayString = "" + EnumChatFormatting.WHITE + "> "
				...
				this.drawString(this.fontRenderer, this.options.getKeyBindingDescription(var5),
						var4 + var5 % 2 * 160 + 70 + 6, this.height / 6 + 20 * (var5 >> 1) + 7, -1);
				++var5;
				break;
			}
		}

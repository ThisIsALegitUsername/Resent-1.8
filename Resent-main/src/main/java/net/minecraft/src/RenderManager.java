...
public class RenderManager {
	 ...
	 */
	public void cacheActiveRenderInfo(World par1World, RenderEngine par2RenderEngine, FontRenderer par3FontRenderer,
			EntityLiving par4EntityLiving, EntityLiving par5EntityLiving, GameSettings par6GameSettings, float par7) {
		...
		if (par4EntityLiving.isPlayerSleeping()) {
			int var8 = par1World.getBlockId(MathHelper.floor_double(par4EntityLiving.posX),
					MathHelper.floor_double(par4EntityLiving.posY), MathHelper.floor_double(par4EntityLiving.posZ));

			if (var8 == Block.bed.blockID) {
				int var9 = par1World.getBlockMetadata(MathHelper.floor_double(par4EntityLiving.posX),
						MathHelper.floor_double(par4EntityLiving.posY), MathHelper.floor_double(par4EntityLiving.posZ));
				int var10 = var9 & 3;
			...
			this.playerViewY = par4EntityLiving.prevRotationYaw
					+ (par4EntityLiving.rotationYaw - par4EntityLiving.prevRotationYaw) * par7;
			this.playerViewX = par4EntityLiving.prevRotationPitch
					+ (par4EntityLiving.rotationPitch - par4EntityLiving.prevRotationPitch) * par7;
		}
		...
		this.viewerPosX = par4EntityLiving.lastTickPosX
				+ (par4EntityLiving.posX - par4EntityLiving.lastTickPosX) * (double) par7;
		this.viewerPosY = par4EntityLiving.lastTickPosY
				+ (par4EntityLiving.posY - par4EntityLiving.lastTickPosY) * (double) par7;
		this.viewerPosZ = par4EntityLiving.lastTickPosZ
				+ (par4EntityLiving.posZ - par4EntityLiving.lastTickPosZ) * (double) par7;
	}
	 ...
	 */
	public void renderEntityWithPosYaw(Entity par1Entity, double par2, double par4, double par6, float par8,
			float par9) {
		...
		if (var10 != null && renderEngine != null && !par1Entity.isInvisible()) {
			if (W.hitboxes().isEnabled()) {
			...
			Minecraft.getMinecraft().renderGlobal.drawOutlinedBoundingBox(
					new AxisAlignedBB(
							par1Entity.boundingBox.minX
									- 0.05
									- par1Entity.posX
									+ (par1Entity.posX - renderPosX),
							par1Entity.boundingBox.minY
									- 0.05
									- par1Entity.posY
									+ (par1Entity.posY - renderPosY),
							par1Entity.boundingBox.minZ
									- 0.05
									- par1Entity.posZ
									+ (par1Entity.posZ - renderPosZ),
							par1Entity.boundingBox.maxX
									+ 0.05
									- par1Entity.posX
									+ (par1Entity.posX - renderPosX),
							par1Entity.boundingBox.maxY
									+ 0.1
									- par1Entity.posY
									+ (par1Entity.posY - renderPosY),
							par1Entity.boundingBox.maxZ
									+ 0.05
									- par1Entity.posZ
									+ (par1Entity.posZ - renderPosZ)));

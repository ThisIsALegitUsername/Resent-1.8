...
import dev.resent.util.misc.ArrayUtils;
...
public class GameSettings {
	...
	private static final String[] CHAT_VISIBILITIES = new String[] { "options.chat.visibility.full",
			"options.chat.visibility.system", "options.chat.visibility.hidden" };
	...
	private static final String[] AMBIENT_OCCLUSIONS = new String[] { "options.ao.off", "options.ao.min",
			"options.ao.max" };
	private static final String[] ANTIALIASING = new String[] { "options.framebufferAntialias.none",
			"options.framebufferAntialias.auto", "options.framebufferAntialias.fxaa",
			"options.framebufferAntialias.msaa4", "options.framebufferAntialias.msaa8" };
	...
	public KeyBinding keyBindFreelook = new KeyBinding("Freelook", Keyboard.KEY_L);
	public KeyBinding keyBindClickGui = new KeyBinding("ClickGui", Keyboard.KEY_Y);
	public KeyBinding[] keyBindsHotbar = new KeyBinding[] { new KeyBinding("Slot 1", 2), new KeyBinding("Slot 2", 3),
			new KeyBinding("Slot 3", 4), new KeyBinding("Slot 4", 5), new KeyBinding("Slot 5", 6),
			new KeyBinding("Slot 6", 7), new KeyBinding("Slot 7", 8), new KeyBinding("Slot 8", 9),
			new KeyBinding("Slot 9", 10) };
	...
	public boolean allowFNAWSkins = false;
	...
	public GameSettings(Minecraft par1Minecraft) {
		this.keyBindings = (KeyBinding[]) ArrayUtils.addAll(
				new KeyBinding[] { this.keyBindAttack, this.keyBindUseItem, this.keyBindForward, this.keyBindLeft,
						...
						this.keyBindSprint, this.keyBindZoom, this.keyBindFunction, this.keyBindFreelook,
						this.keyBindClickGui },
				this.keyBindsHotbar);
		this.difficulty = 1;
		this.hideGUI = false;
	...
	public static String getKeyDisplayString(int par0) {
		return par0 < 0
				? StatCollector.translateToLocalFormatted("key.mouseButton",
						new Object[] { Integer.valueOf(par0 + 101) })
				: EaglerAdapter.getKeyName(par0);
	}
	...
	public static boolean isKeyDown(KeyBinding par0KeyBinding) {
		return par0KeyBinding.keyCode < 0 ? EaglerAdapter.mouseIsButtonDown(par0KeyBinding.keyCode + 100)
				: EaglerAdapter.isKeyDown(par0KeyBinding.keyCode);
	}
	...
	public void setOptionValue(EnumOptions par1EnumOptions, int par2) {
		...
		if (par1EnumOptions == EnumOptions.RENDER_DISTANCE) {
			...
			if (this.mc.isSingleplayerOrLAN()) {
				Minecraft.getMinecraft().displayEaglercraftText(
						"changing render distance in singleplayer may take a few seconds, this is not a glitch plz don't report");
			}
		}
	...
	public String getKeyBinding(EnumOptions par1EnumOptions) {
		...
		if (par1EnumOptions.getEnumFloat()) {
			...
			return par1EnumOptions == EnumOptions.SENSITIVITY
					? (var5 == 0.0F ? var3 + var2.translateKey("options.sensitivity.min")
							: (var5 == 1.0F ? var3 + var2.translateKey("options.sensitivity.max")
									: var3 + (int) (var5 * 200.0F) + "%"))
					: (par1EnumOptions == EnumOptions.FOV
							? (var5 == 0.0F ? var3 + var2.translateKey("options.fov.min")
									: (var5 == 1.0F ? var3 + var2.translateKey("options.fov.max")
											: var3 + (int) (70.0F + var5 * 40.0F)))
							: (par1EnumOptions == EnumOptions.GAMMA
									? (var5 == 0.0F ? var3 + var2.translateKey("options.gamma.min")
											: (var5 == 1.0F ? var3 + var2.translateKey("options.gamma.max")
									...
									: (par1EnumOptions == EnumOptions.CHAT_VISIBILITY
											...
											: (par1EnumOptions == EnumOptions.PARTICLES
													...
													: (par1EnumOptions == EnumOptions.FRAMERATE_LIMIT
															? var3 + getTranslation(LIMIT_FRAMERATES,
																	this.limitFramerate)
															: (par1EnumOptions == EnumOptions.AMBIENT_OCCLUSION
																	? var3 + getTranslation(AMBIENT_OCCLUSIONS,
																			this.ambientOcclusion)
																	: (par1EnumOptions == EnumOptions.ANTIALIASING
																			? var3 + getTranslation(ANTIALIASING,
																					this.antialiasMode)
																			: (par1EnumOptions == EnumOptions.CHUNK_UPDATES
																					? var3 + (chunkUpdatePerFrame + 1)
																					: (par1EnumOptions == EnumOptions.GRAPHICS
																							? (this.fancyGraphics ? var3
																									+ var2.translateKey(
	...
	public void loadOptions() {
		...
		if (!yee.hasNoTags()) {
			...
			Resent.INSTANCE.loadSettings(yee);

			if (yee.hasKey("musicVolume"))
				this.musicVolume = yee.getFloat("musicVolume");
			if (yee.hasKey("soundVolume"))
				this.soundVolume = yee.getFloat("soundVolume");
			if (yee.hasKey("sensitivity"))
				this.mouseSensitivity = yee.getFloat("sensitivity");
			if (yee.hasKey("fov"))
				this.fovSetting = yee.getFloat("fov");
			if (yee.hasKey("gamma"))
				this.gammaSetting = yee.getFloat("gamma");
			if (yee.hasKey("invertMouse"))
				this.invertMouse = yee.getBoolean("invertMouse");
			if (yee.hasKey("viewDistance"))
				this.renderDistance = yee.getInteger("viewDistance");
			if (yee.hasKey("guiScale"))
				this.guiScale = yee.getInteger("guiScale");
			if (yee.hasKey("particles"))
				this.particleSetting = yee.getInteger("particles");
			if (yee.hasKey("viewBobbing"))
				this.viewBobbing = yee.getBoolean("viewBobbing");
			if (yee.hasKey("anaglyph"))
				this.anaglyph = yee.getBoolean("anaglyph");
			if (yee.hasKey("limitFramerate"))
				this.limitFramerate = yee.getInteger("limitFramerate");
			if (yee.hasKey("fancyGraphics"))
				this.fancyGraphics = yee.getBoolean("fancyGraphics");
			if (yee.hasKey("ambientOcclusion"))
				this.ambientOcclusion = yee.getInteger("ambientOcclusion");
			if (yee.hasKey("clouds"))
				this.clouds = yee.getBoolean("clouds");
			if (yee.hasKey("fog"))
				this.enableFog = yee.getBoolean("fog");
			if (yee.hasKey("lastServer"))
				this.lastServer = yee.getString("lastServer");
			if (yee.hasKey("language"))
				this.language = yee.getString("language");
			if (yee.hasKey("chatVisibility"))
				this.chatVisibility = yee.getInteger("chatVisibility");
			if (yee.hasKey("chatColours"))
				this.chatColours = yee.getBoolean("chatColours");
			if (yee.hasKey("chatLinks"))
				this.chatLinks = yee.getBoolean("chatLinks");
			if (yee.hasKey("chatLinksPrompt"))
				this.chatLinksPrompt = yee.getBoolean("chatLinksPrompt");
			if (yee.hasKey("chatOpacity"))
				this.chatOpacity = yee.getFloat("chatOpacity");
			if (yee.hasKey("fullScreen"))
				this.fullScreen = yee.getBoolean("fullScreen");
			if (yee.hasKey("hideServerAddress"))
				this.hideServerAddress = yee.getBoolean("hideServerAddress");
			if (yee.hasKey("advancedTooltips"))
				this.advancedItemTooltips = yee.getBoolean("advancedTooltips");
			if (yee.hasKey("pauseOnLostFocus"))
				this.pauseOnLostFocus = yee.getBoolean("pauseOnLostFocus");
			if (yee.hasKey("showCape"))
				this.showCape = yee.getBoolean("showCape");
			if (yee.hasKey("touchscreen"))
				this.touchscreen = yee.getBoolean("touchscreen");
			if (yee.hasKey("forceHeight"))
				this.overrideHeight = yee.getInteger("forceHeight");
			if (yee.hasKey("forceWidth"))
				this.overrideWidth = yee.getInteger("forceWidth");
			if (yee.hasKey("antialiasMode"))
				this.antialiasMode = yee.getInteger("antialiasMode");
			if (yee.hasKey("heldItemTooltips"))
				this.heldItemTooltips = yee.getBoolean("heldItemTooltips");
			if (yee.hasKey("chatHeightFocused"))
				this.chatHeightFocused = yee.getFloat("chatHeightFocused");
			if (yee.hasKey("chatHeightUnfocused"))
				this.chatHeightUnfocused = yee.getFloat("chatHeightUnfocused");
			if (yee.hasKey("chatScale"))
				this.chatScale = yee.getFloat("chatScale");
			if (yee.hasKey("chatWidth"))
				this.chatWidth = yee.getFloat("chatWidth");
			if (yee.hasKey("showCoordinates"))
				this.showCoordinates = yee.getBoolean("showCoordinates");
			if (yee.hasKey("showSkinJacket"))
				showSkinJacket = yee.getBoolean("showSkinJacket");
			if (yee.hasKey("showSkinHat"))
				showSkinHat = yee.getBoolean("showSkinHat");
			if (yee.hasKey("showSkinLeftArm"))
				showSkinLeftArm = yee.getBoolean("showSkinLeftArm");
			if (yee.hasKey("showSkinRightArm"))
				showSkinRightArm = yee.getBoolean("showSkinRightArm");
			if (yee.hasKey("showSkinLeftLeg"))
				showSkinLeftLeg = yee.getBoolean("showSkinLeftLeg");
			if (yee.hasKey("showSkinRightLeg"))
				showSkinRightLeg = yee.getBoolean("showSkinRightLeg");
			if (yee.hasKey("allowFNAWSkins"))
				allowFNAWSkins = yee.getBoolean("allowFNAWSkins");
			if (yee.hasKey("showOtherCapes"))
				showOtherCapes = yee.getBoolean("showOtherCapes");
			if (yee.hasKey("chunkUpdates"))
				chunkUpdatePerFrame = yee.getInteger("chunkUpdates");
			if (yee.hasKey("voiceListenVolume"))
				voiceListenVolume = yee.getFloat("voiceListenVolume");
			if (yee.hasKey("voiceSpeakVolume"))
				voiceSpeakVolume = yee.getFloat("voiceSpeakVolume");
			if (yee.hasKey("voicePTTKey"))
				voicePTTKey = yee.getInteger("voicePTTKey");
			if (yee.hasKey("voiceListenRadius"))
				voiceListenRadius = yee.getInteger("voiceListenRadius");
			if (yee.hasKey("difficulty"))
				difficulty = yee.getByte("difficulty");
			if (yee.hasKey("hideJoinCode"))
				hideJoinCode = yee.getBoolean("hideJoinCode");
			if (yee.hasKey("relayTimeout"))
				relayTimeout = yee.getByte("relayTimeout");
			if (yee.hasKey("adderall"))
				adderall = yee.getBoolean("adderall");

			if (voiceListenRadius < 5)
				voiceListenRadius = 5;
			else if (voiceListenRadius > 22)
				voiceListenRadius = 22;
	...
	public void saveOptions() {
		...
		Resent.INSTANCE.saveSettings(yee);
	...
	public void sendSettingsToServer() {
		if (this.mc.thePlayer != null) {
			this.mc.thePlayer.sendQueue.addToSendQueue(new Packet204ClientInfo(this.language, this.renderDistance,
					this.chatVisibility, this.chatColours, this.difficulty, this.showCape));
			this.mc.thePlayer.sendQueue.addToSendQueue(
					new Packet250CustomPayload("EAG|SkinLayers", new byte[] { (byte) getSkinLayers() }));
		}
	}

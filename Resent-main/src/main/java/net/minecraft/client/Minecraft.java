...
public class Minecraft implements Runnable {
	...
	public void startGame() {
		...
		if (s != null) {
			scr = new GuiScreenEditProfile(
					new GuiConnecting(new GuiMainMenu(), this, new ServerData("Eaglercraft Server", s, false)));
		} else {
	...
	public void stopServerAndDisplayGuiScreen(GuiScreen par1GuiScreen) {
		if (!IntegratedServer.isWorldNotLoaded()) {
			...
			displayGuiScreen(
					new GuiScreenSingleplayerLoading(par1GuiScreen, "saving world", () -> IntegratedServer.isReady()));
		} else {
	...
	private void runGameLoop() {
		...
		if (!this.fullscreen && (EaglerAdapter.getCanvasWidth() != this.displayWidth
		...
		if (this.mcProfiler.profilingEnabled) {
			...
			for (int var21 = 0; var21 < var3.size(); ++var21) {
				...
				this.fontRenderer.drawStringWithShadow(var22, var7 - var6, var8 + var6 / 2 + var21 * 8 + 20,
						var20.func_76329_a());
				this.fontRenderer.drawStringWithShadow(var22 = var18.format(var20.field_76332_a) + "%",
						var7 + var6 - 50 - this.fontRenderer.getStringWidth(var22), var8 + var6 / 2 + var21 * 8 + 20,
						var20.func_76329_a());
				this.fontRenderer.drawStringWithShadow(var22 = var18.format(var20.field_76330_b) + "%",
	...
	public void runTick() {
		...
		this.isGamePaused = this.isSingleplayer() && this.theWorld != null && this.thePlayer != null
		...
		if (lanState && !IntegratedServerLAN.isLANOpen()) {
			lanState = false;
			if (thePlayer != null) {
				thePlayer.sendChatToPlayer(
						EnumChatFormatting.RED + StatCollector.translateToLocal("lanServer.relayDisconnected"));
			}
		}
		...
		if (EaglerAdapter.getVoiceStatus() == Voice.VoiceStatus.CONNECTING
				|| EaglerAdapter.getVoiceStatus() == Voice.VoiceStatus.CONNECTED) {

			EaglerAdapter.activateVoice((this.currentScreen == null || !this.currentScreen.blockHotKeys())
					&& EaglerAdapter.isKeyDown(gameSettings.voicePTTKey));

			if (this.theWorld != null && this.thePlayer != null) {
				HashSet<String> seenPlayers = new HashSet<>();
				for (Object playerObject : this.theWorld.playerEntities) {
					EntityPlayer player = (EntityPlayer) playerObject;
					if (player == this.thePlayer)
						continue;
					if (EaglerAdapter.getVoiceChannel() == Voice.VoiceChannel.PROXIMITY)
						EaglerAdapter.updateVoicePosition(player.username, player.posX,
								player.posY + player.getEyeHeight(), player.posZ);
					int prox = 22;
					// cube
					if (Math.abs(thePlayer.posX - player.posX) <= prox && Math.abs(thePlayer.posY - player.posY) <= prox
							&& Math.abs(thePlayer.posZ - player.posZ) <= prox) {
		...
		if (this.currentScreen == null || this.currentScreen.allowUserInput) {
			...
			while (EaglerAdapter.mouseNext()) {
				KeyBinding.setKeyBindState(EaglerAdapter.mouseGetEventButton() - 100,
				...
				if (var1 <= 200L) {
					...
					if (this.currentScreen == null) {
						if ((!this.inGameHasFocus || !EaglerAdapter.isPointerLocked())
								&& EaglerAdapter.mouseGetEventButtonState()) {
							this.setIngameFocus();
						}
			...
			while (EaglerAdapter.keysNext()) {
				...
				EventKey event = new EventKey(EaglerAdapter.getEventKey());
				...
				if (EaglerAdapter.getEventKeyState()) {
					isGonnaTakeDatScreenShot |= (this.gameSettings.keyBindFunction.pressed
							&& EaglerAdapter.getEventKey() == 3);
					if (EaglerAdapter.getEventKey() == 87) {
						...
						if (this.currentScreen != null) {
							...
							Resent.INSTANCE.modManager.onKey(EaglerAdapter.getEventKey());
							if(EaglerAdapter.getEventKey() == this.gameSettings.keyBindFreelook.keyCode)
							W.freelook().smh();
						...
						if (this.gameSettings.showDebugInfo && this.gameSettings.showDebugProfilerChart
								&& !this.gameSettings.keyBindFunction.pressed) {
				...
				if(this.gameSettings.keyBindClickGui.pressed){
			...
			if (this.gameSettings.keyBindSprint.pressed && !this.thePlayer.isSprinting() && this.thePlayer.canSprint()
					&& !this.thePlayer.isCollidedHorizontally) {
		...
		if (this.theWorld != null) {
			if (this.thePlayer != null) {
				...
				if (messageOnLoginCounter == 100 && isSingleplayerOrLAN()) {
					displayEaglercraftText(
							EnumChatFormatting.GREEN + "Notice: chunk loading may take a while in singleplayer.");
				}

				if (messageOnLoginCounter == 150 && isSingleplayerOrLAN()) {
					displayEaglercraftText(EnumChatFormatting.AQUA
							+ "Especially in new worlds, if no chunks show give the game up to 5 straight minutes before \"giving up\" on a new world");
				}
			}
	...
	public void loadWorld(WorldClient par1WorldClient, String par2Str) {
		...
		if (par1WorldClient != null) {
			...
			// displayEaglercraftText("ANGLE Issue #4994 is unpatched on this browser, using
			// fake aliased sampling on linear magnified terrain texture for anisotropic
			// filtering. Chrome patch progress and information available at
			// https://crbug.com/angleproject/4994");
			// }
			...
			if (this.gameSettings.showCoordinates) {
				displayEaglercraftText(EnumChatFormatting.LIGHT_PURPLE
						+ "Note: use F+6 to hide the coordinates off of the screen (if you're in public)");
			} else {
				displayEaglercraftText(
						EnumChatFormatting.LIGHT_PURPLE + "Note: use F+6 to show your coordinates on the screen");
			}
	...
	public void launchIntegratedServer(String folderName, String trim, WorldSettings var6) {
		...
		this.displayGuiScreen(new GuiScreenSingleplayerLoading(
				...
				"Loading world: " + folderName, () -> IntegratedServer.isWorldRunning()));
	}
}
